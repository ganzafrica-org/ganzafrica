...
import { MoreVertical, Calendar, Mail, Phone, MapPin, TrendingUp, Bot } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <div className={`flex gap-6 overflow-x-auto pb-4 ${className}`}>
      {columns.map((column) => (
        ...
        >
          <div
            className={`rounded-xl shadow-sm transition-all duration-200 ${dragOverColumn === column.id ? "ring-2 ring-blue-400 ring-opacity-50 shadow-lg" : ""}`}
          >
                      ...
                      <div className="space-y-3">
                        ...
                        {item.metadata?.cvScore && (
                          <div className="flex items-center gap-2">
                            ...
                            <Badge
                              className={`text-xs ${
                                item.metadata.cvScore >= 80
                                  ? "bg-emerald-100 text-emerald-700"
                                  : item.metadata.cvScore >= 60
