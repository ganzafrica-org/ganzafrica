...
const NewsDetailsContent = () => {
      ...
      return htmlContent
        .replace(/(<p>)(.*?)(<\/p>)/gs, (match, openTag, content, closeTag) => {
          ...
          sentences.forEach((sentence: string) => {
            ...
            const sentenceWordCount: number =
              trimmedSentence.split(/\s+/).length;
        ...
        .replace(
          /<h([1-6])>(.*?)<\/h\1>/g,
          (match, level, content) =>
            `<h${level} class="mt-8 mb-4">${content}</h${level}>`,
        );
    } else {
  ...
  const getMediaItems = (article: ArticleWithMedia): MediaItem[] => {
    try {
      ...
      return [...article.media.items].sort(
        ...
        >
          ...
          {selectedMedia.type === "image" ? (
            ...
            <video
              ...
              onPlay={() =>
                trackVideoEvent("play", article?.title || "Unknown Video")
              }
              onPause={() =>
                trackVideoEvent("pause", article?.title || "Unknown Video")
              }
              onEnded={() =>
                trackVideoEvent("complete", article?.title || "Unknown Video")
              }
            />
          ) : (
        ...
        <Container className="py-16">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-8">
              The article you're looking for may have been removed or doesn't
              exist.
            </p>
            <SafeLink
              href={`/newsroom`}
  ...
  useEffect(() => {
    const translateArticle = async () => {
      ...
      // If same as source language, reset to original
  ...
  // Translation handlers for manual translate button
  ...
  // Get the current content to display (translated or original)
  const isTranslated =
    targetLanguage !== sourceLanguage && (translatedTitle || translatedContent);
  const displayTitle =
  ...
  const displayContent =
    isTranslated && translatedContent
      ...
      : article.content ||
        article.description ||
        <TranslatableText>{"No content available for this article."}</TranslatableText>;
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 md:p-8">
              ...
              {isTranslated && !isTranslating && (
                <div className="mb-4 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700">
                  This article has been translated. Use the language selector in
                  the navigation to change language.
                </div>
              )}
