...
export default function GlobalNotFound() {
  ...
  const texts: Record<string, { title: string; message: string; requested: string; goHome: string; switchTo: string }>= {
    en: {
      title: 'Oops!',
      ...
      <div className="max-w-6xl w-full flex flex-col lg:flex-row items-center justify-between gap-12">
        <div className={`flex-1 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
          <div className="retro-tv-container">
            <Image src={ganzalogo} alt="Retro TV" width={400} height={380} className='w-[70rem] h-[35rem] border border-black' />
          </div>
        </div>

        <div className={`flex-1 text-center lg:text-left transition-all duration-1000 text-white ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
          ...
          <p className="text-sm text-gray-400 mb-6">{t.requested} <span className="font-mono">{pathname}</span></p>
