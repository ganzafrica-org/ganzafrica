...
export default function FellowshipPage() {
    ...
    const [featuredOpportunity, setFeaturedOpportunity] = useState(null);
    const videoRef = useRef(null);
    const [isMuted, setIsMuted] = useState(true);
    const [currentTestimonial, setCurrentTestimonial] = useState(0);
    const [testimonials, setTestimonials] = useState(fallbackTestimonials); // Initialize with fallback
    ...
    const nextTestimonial = () => {
        if (isAnimating)
            return; // Prevent rapid clicks during animation
    ...
    const handleTestimonialChange = (index) => {
        if (isAnimating || index === currentTestimonial)
            return; // Skip if animating or already on this testimonial
        setIsAnimating(true);
    ...
    return (<div className="min-h-screen bg-white font-sans">
        ...
        <section className="relative h-[400px] md:h-[500px]">
          ...
          <div className="relative z-10 h-full flex items-center justify-center">
            <div className="container mx-auto px-4 text-center">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="flex items-center justify-center gap-4">
        ...
        <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="py-16 md:py-20 bg-gray-100 relative overflow-hidden">
          {/* Background Pattern */}
          ...
          <div className="absolute left-0 top-1/4 -translate-x-1/4 opacity-10 hidden sm:block">
            <img src="/images/leaf.png" alt="Decorative leaf" className="w-32 h-32 sm:w-48 sm:h-48 md:w-64 md:h-64 transform -rotate-12"/>
          </div>
          ...
          <div className="container mx-auto px-4 sm:px-6 md:px-12 lg:px-20 relative z-10">
            <div className="relative flex flex-col md:block">
              <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="w-full md:w-[60%] mb-8 md:mb-0">
                <div className="relative">
                  <div className="w-full h-[300px] sm:h-[400px] md:h-[500px] rounded-lg overflow-hidden">
                    <video ref={videoRef} className="w-full h-full object-cover" src="/videos/farmer1.mp4" autoPlay muted loop playsInline/>
                    {/* Video Controls */}
                    <div className="absolute bottom-4 right-4 flex space-x-3">
                      <button onClick={toggleMute} className="bg-black bg-opacity-60 hover:bg-opacity-80 text-white p-2 rounded-full transition-all" aria-label={isMuted ? "Unmute video" : "Mute video"}>
                        {isMuted ? <AudioMutedIcon /> : <AudioUnmutedIcon />}
                      </button>
                      <button onClick={toggleFullScreen} className="bg-black bg-opacity-60 hover:bg-opacity-80 text-white p-2 rounded-full transition-all" aria-label="View in full screen">
                        <FullscreenIcon />
                      </button>
                    </div>
                  </div>
              ...
              <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.8 }} className="w-full md:absolute md:top-12 md:right-0 md:w-[50%] bg-white p-4 sm:p-6 rounded-lg shadow-lg">
                <DecoratedHeading firstText="About the" secondText="Fellowship" className='mb-4 sm:mb-5'/>
                ...
                <Link href={`/${locale}/programs/fellowship/how-to-apply`}>
                  <motion.button className="bg-[#045F3C] hover:bg-[#045F3C]/90 text-white px-6 py-3 rounded-md font-medium transition-colors" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    How to Apply
                  </motion.button>
                </Link>
              </motion.div>
        ...
        <motion.section initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="py-16 md:py-20 bg-white">
          <div className="container mx-auto px-6 md:px-12 lg:px-20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.8 }} className="order-2 md:order-1">
                <img src="/images/SHIR5142-Enhanced-NR.jpg" alt="Food System" className="rounded-lg w-full h-[500px] object-cover"/>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.8 }} className="order-1 md:order-2">
                <DecoratedHeading firstText="Discover tomorrow's" secondText="leaders today"/>
                ...
                <Link href={`/${locale}/about/team`}>
                  <motion.button className="bg-primary-orange hover:bg-yellow-500 text-white px-8 py-3 rounded-md font-medium transition-colors" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    Meet the Fellows
                  </motion.button>
                </Link>
              </motion.div>
        ...
        <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="py-12 md:py-16 bg-white">
          <div className="container mx-auto px-6 md:px-12 lg:px-16">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-center max-w-2xl mx-auto mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                {benefits.slice(0, 2).map((benefit, index) => (<motion.div key={benefit.title} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.2, duration: 0.8 }} className="bg-white rounded-[16px] overflow-hidden border border-gray-100">
                      <div className="p-3">
                        <div className="rounded-xl overflow-hidden relative">
                          <img src={benefit.image} alt={benefit.title} width={600} height={200} className="w-full h-[200px] object-cover" style={{ objectPosition: "center center" }}/>
                        </div>
                      </div>
                      <div className="p-6">
              ...
              {/* Right Column */}
              <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.8 }} className="bg-white rounded-[16px] overflow-hidden border border-gray-100 h-full">
                <div className="p-3">
                  <div className="rounded-xl overflow-hidden relative h-[600px]">
                    <Image src="/images/ganzafrica-fellows.jpg" alt="Develop Your Skills" width={800} height={600} className="w-full h-full object-cover" style={{
        ...
        <motion.section initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="py-24 md:py-32 bg-[#045F3C] relative overflow-hidden">
          <div className="container mx-auto px-6 md:px-12 lg:px-20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
              <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.8 }} className="max-w-xl">
              ...
              <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.8 }} className="space-y-8 relative">
                {/* Dotted Line Connector */}
                <div className="absolute left-6 top-[24px] bottom-[24px] border-l-2 border-dashed border-white/20"></div>

                {promises.map((promise, index) => (<motion.div key={promise.number} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 * index, duration: 0.8 }} className="flex items-start gap-6 relative">
        ...
        <motion.section initial={{ opacity: 1 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="py-12 md:py-16 bg-gray-50">
          <Container>
            ...
            {loading && testimonials.length === 0 ? (<div className="max-w-5xl mx-auto px-4 md:px-12">
                  {/* Loading skeleton */}
                  <div className="flex justify-center items-center gap-4 md:gap-6 mb-6 md:mb-8">
                    {[1, 2, 3, 4, 5].map((_, index) => (<div key={index} className="w-14 md:w-20 h-14 md:h-20 rounded-full bg-gray-200 animate-pulse"/>))}
                  </div>
                ...
                </div>) : (<div className="max-w-5xl mx-auto px-4 md:px-12">
                  {/* Profile Images Row */}
                ...
                return (<div key={testimonial.id} className={`cursor-pointer transition-all duration-300 transform ${isActive
                        ? 'w-14 md:w-20 h-14 md:h-20 z-20 scale-110'
                        : isPrevious || isNext
                            ? 'w-10 md:w-16 h-10 md:h-16 z-10 opacity-70 scale-90'
                            : 'w-8 md:w-12 h-8 md:h-12 opacity-50 scale-75'}`} onClick={() => handleTestimonialChange(index)}>
                            <div className={`rounded-full overflow-hidden h-full w-full transition-all duration-300 ${isActive ? 'ring-2 md:ring-4 ring-yellow-400' : 'ring-1 ring-gray-200'}`}>
                              <img src={testimonial.image} alt={testimonial.author_name} width={80} height={80} className="w-full h-full object-cover"/>
                            </div>
                          </div>);
            })}
                  </div>
                  ...
                  <div className="relative">
                    <div className="bg-white p-6 md:p-8 rounded-lg shadow-sm min-h-[250px] flex flex-col justify-center">
                      <div className="text-center px-2 md:px-16">
                        <motion.div key={currentTestimonial} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.5 }} className="min-h-[120px] md:min-h-[150px] flex items-center justify-center mb-4 md:mb-6">
                          <p className="text-gray-600 text-sm md:text-base leading-relaxed max-w-2xl mx-auto">
                            {testimonials[currentTestimonial]?.description || testimonials[0]?.description}
                          </p>
                        </motion.div>

                        <motion.div key={`name-${currentTestimonial}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.5, delay: 0.1 }} className="min-h-[50px] md:min-h-[60px]">
                    ...
                    <button onClick={prevTestimonial} className="absolute left-0 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full text-primary-orange flex items-center justify-center hover:bg-yellow-500 transition-colors -translate-x-1/2 md:-translate-x-5" aria-label="Previous testimonial">
                      <ArrowLeft className="w-5 h-5 text-white"/>
                    </button>
                    <button onClick={nextTestimonial} className="absolute right-0 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-[#045F3C] text-white flex items-center justify-center hover:bg-[#034830] transition-colors translate-x-1/2 md:translate-x-5" aria-label="Next testimonial">
