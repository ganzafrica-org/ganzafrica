...
const NavigationItem = ({ label, isActive, onClick, count, }) => (<button onClick={onClick} className={`relative group flex items-center gap-2 px-4 py-2 font-bold text-lg transition-colors duration-300 ${isActive ? "text-black" : "text-gray-600 hover:text-black"}`}>
      {label !== "All" && (<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {label}
      {count !== undefined && (<span className={`ml-1 px-2 py-0.5 text-xs font-medium rounded-full ${isActive ? "bg-[#FFB800] text-white" : "bg-gray-200 text-gray-600"}`}>
        {count}
      </span>)}
      {isActive && (<div className="absolute bottom-0 left-0 right-0 h-1 bg-[#FFB800] rounded-full"/>)}
      {!isActive && (<div className="absolute bottom-0 left-0 right-0 h-1 bg-[#FFB800] rounded-full scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"/>)}
    </button>);
const NewsCard = ({ item, locale }) => {
    ...
    const getCoverImage = () => {
        try {
            ...
            const coverImage = item.media.items.find(mediaItem => mediaItem.cover === true && mediaItem.type === 'image');
    ...
    return (<Link href={`/${locale}/newsroom/${slug}`} className="block group">
        <div className="relative bg-white rounded-[24px] overflow-hidden transition-all duration-500 ease-out hover:-translate-y-2 hover:shadow-2xl border border-gray-200">
          <div className="relative aspect-[16/10]">
            ...
            <div className="w-full h-full overflow-hidden">
              {imageUrl ? (<div className="w-full h-full relative" style={{ transform: 'translateY(-15%)' }}>
                    <img src={imageUrl} alt={item.title} className="w-full h-[130%] object-cover transition-transform duration-700 ease-out group-hover:scale-[1.08]"/>
...
const NewsroomPage = () => {
    ...
    useEffect(() => {
        if (activeFilter === "all") {
            ...
            const filtered = allNews.filter(article => article.tags &&
  ...
  <div className="absolute inset-0 z-0">
    <div className="relative w-full h-[130%]" style={{ transform: 'translateY(-18%)' }}>
      <Image src="/images/DSC_1430.JPG" alt="Ganz Africa Newsroom" fill className="object-cover object-top" priority/>
    </div>
  </div>
    ...
    Sharing Our Progress and  Transformative Impact  
        ...
        <Container className="py-12">
          ...
          <nav className="mb-12 flex items-center justify-center space-x-12 overflow-x-auto pb-4 scrollbar-hide">
            <NavigationItem label="All" isActive={activeFilter === "all"} onClick={() => setActiveFilter("all")} count={tagCounts.all}/>
            {tags.map(tag => (<NavigationItem key={tag.id} label={tag.name} isActive={activeFilter === tag.id.toString()} onClick={() => setActiveFilter(tag.id.toString())} count={tagCounts[tag.id] || 0}/>))}
          </nav>
          ...
          {loading ? (<div className="flex justify-center items-center min-h-[400px]">
                ...
                {filteredNews.map((item) => (<NewsCard key={item.id} item={item} locale={locale}/>))}
              </div>)}
        </Container>
      </main>);
};
