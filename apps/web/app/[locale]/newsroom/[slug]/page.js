...
const NewsDetailsPage = () => {
    ...
    const processContentWithWordBreaks = (htmlContent) => {
        ...
        if (isHTML) {
            ...
            return htmlContent.replace(/(<p>)(.*?)(<\/p>)/gs, (match, openTag, content, closeTag) => {
            ...
            }).replace(/<h([1-6])>(.*?)<\/h\1>/g, (match, level, content) => `<h${level} class="mt-8 mb-4">${content}</h${level}>`);
        }
        else {
    ...
    const processPlainTextWithWordBreaks = (text) => {
        ...
        return (<div>
          {paragraphs.map((paragraph, index) => {
                ...
                return (<p key={index} className="text-lg leading-relaxed mb-6">
                  {chunks.map((chunk, chunkIndex) => (<React.Fragment key={chunkIndex}>
                        {chunk}
        ...
        const fetchArticleBySlug = async () => {
            try {
                ...
                if (response.data && response.data.news) {
                    ...
                    const foundArticle = articles.find((article) => generateSlug(article.title) === slug);
                    if (foundArticle) {
                        setArticle(foundArticle);
                        // Get related articles (same tag)
                        if (foundArticle.tags && foundArticle.tags.length > 0) {
                            const mainTag = foundArticle.tags[0];
                            const related = articles.filter((a) => a.id !== foundArticle.id &&
                                a.tags &&
                                a.tags.some((tag) => tag.id === mainTag.id)).slice(0, 3); // Get up to 3 related articles
    ...
    const getCoverImage = (article) => {
        try {
            ...
            const coverImage = article.media.items.find((mediaItem) => mediaItem.cover === true && mediaItem.type === 'image');
            // Return the cover image URL if found
            if (coverImage && coverImage.url) {
                return coverImage.url;
            }
            // Fallback to the first image if no cover image is specified
            const firstImage = article.media.items.find(mediaItem => mediaItem.type === 'image');
    ...
    const RelatedArticleCard = ({ item }) => {
        ...
        return (<Link href={`/${locale}/newsroom/${itemSlug}`} className="block group">
          <div className="relative bg-white rounded-lg overflow-hidden shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="relative aspect-[16/10]">
              {imageUrl ? (<img src={imageUrl} alt={item.title} className="w-full h-full object-contain bg-gray-100"/>) : (<div className="w-full h-full bg-gray-200 flex items-center justify-center">
                    <span className="text-gray-400">No image</span>
                  </div>)}

              {/* Tag badge */}
              {item.tags && item.tags.length > 0 && (<div className="absolute bottom-2 left-2 px-3 py-1 bg-[#00A651] text-white rounded-full text-xs font-medium">
    ...
    const MediaThumbnail = ({ mediaItem, index }) => {
        return (<div className="cursor-pointer rounded-lg overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1" onClick={() => handleMediaClick(mediaItem)}>
          {mediaItem.type === 'image' ? (<div className="aspect-[16/10] group">
                <img src={mediaItem.url} alt={`Media ${index + 1}`} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"/>
              </div>) : mediaItem.type === 'video' ? (<div className="relative aspect-[16/10] bg-black group">
                <img src={mediaItem.thumbnailUrl || mediaItem.url} alt={`Video ${index + 1}`} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"/>
              ...
              </div>) : (<div className="aspect-[16/10] bg-gray-200 flex items-center justify-center">
    ...
    const MediaGalleryModal = () => {
        if (!isGalleryOpen || !selectedMedia)
            return null;
        return (<div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={closeGallery}>
          <div className="relative max-w-6xl w-full h-auto mx-auto" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-4 right-4 z-10 w-10 h-10 bg-white/20 rounded-full flex items-center justify-center hover:bg-white/40 transition-colors" onClick={closeGallery}>
              <X className="w-6 h-6 text-white"/>
            </button>

            {selectedMedia.type === 'image' ? (<img src={selectedMedia.url} alt="Media preview" className="max-h-[85vh] max-w-full object-cover mx-auto"/>) : selectedMedia.type === 'video' ? (<video src={selectedMedia.url} controls className="max-h-[85vh] max-w-full mx-auto bg-black" poster={selectedMedia.thumbnailUrl}/>) : (<div className="bg-gray-200 p-10 rounded-lg text-center">
    ...
    return (<main className="bg-gray-50 min-h-screen">
        ...
        <section className="relative h-[50vh] sm:h-[70vh] md:h-[80vh] lg:h-[90vh] w-full overflow-hidden">
          ...
          <div className="absolute inset-0 z-0">
            {mainImageUrl ? (<img src={mainImageUrl} alt={article.title} className="object-cover w-full h-full"/>) : (<div className="w-full h-full bg-gradient-to-r from-[#004D40] to-[#00A651]"></div>)}
          </div>
        ...
        <Container className="py-12">
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="w-full lg:w-2/3">
              ...
              <Link href={`/${locale}/newsroom`} className="inline-flex items-center text-[#00A651] hover:text-[#008f46] mb-6 transition-colors">
                <ArrowLeft className="mr-2 h-5 w-5"/>
                Back to Newsroom
              </Link>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 md:p-8">
                <div className="prose prose-lg max-w-none prose-headings:mb-6 prose-headings:mt-8 prose-p:my-4 prose-p:leading-relaxed prose-img:my-8 prose-img:rounded-lg prose-hr:my-10" dangerouslySetInnerHTML={{
            __html: processContentWithWordBreaks(article.content || article.description || 'No content available for this article.')
        }}/>

                {/* Media Gallery */}
                {allMediaItems.length > 0 && (<div className="mt-10">
                      <h3 className="text-xl font-semibold text-gray-800 mb-4">Media Gallery</h3>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {allMediaItems.map((mediaItem, index) => (<MediaThumbnail key={mediaItem.id || index} mediaItem={mediaItem} index={index}/>))}
                      </div>
                    </div>)}
              </div>
            ...
            <div className="w-full lg:w-1/3">
              {/* Related Articles */}
              {relatedArticles.length > 0 && (<div className="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div className="mt-6 text-center">
                      <Link href={`/${locale}/newsroom`} className="inline-flex items-center justify-center px-5 py-2 text-sm font-medium text-[#00A651] border border-[#00A651] rounded-md hover:bg-[#00A651] hover:text-white transition-colors">
                        View All News
                      </Link>
                    </div>
                  </div>)}
              ...
              {article.highlights && (<div className="bg-white rounded-xl shadow-sm p-6 mt-6">
                    <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center">
                    ...
                    <ul className="space-y-3">
                      {article.highlights.map((highlight, index) => (<li key={index} className="flex items-start">
