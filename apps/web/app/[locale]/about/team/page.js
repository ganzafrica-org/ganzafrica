...
const TeamMemberModal = ({ member, isOpen, onClose }) => {
    ...
    return (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl w-full max-w-[580px] overflow-hidden relative shadow-[0_8px_32px_rgba(0,0,0,0.12)] transform transition-all duration-500 ease-out" onClick={e => e.stopPropagation()}>
        {/* Close Button */}
        <button onClick={onClose} aria-label="Close modal" className="absolute top-5 right-5 p-2 hover:bg-gray-100 rounded-full transition-all duration-200 ease-in-out z-10 group">
        ...
        <div className="px-6">
          ...
          <div className="py-5 border-t border-[#E5E7EB]">
            ...
            <div className={`text-[15px] text-[#4B5563] leading-[1.7] tracking-wide break-words ${shouldScroll ? 'max-h-[200px] overflow-y-auto custom-scrollbar pr-2' : ''}`}>
          ...
          <div className="py-5 border-t border-[#E5E7EB]">
            ...
            <div className="flex items-center gap-4">
              ...
              {member.profile_link ? (<a href={member.profile_link} target="_blank" rel="noopener noreferrer" className="group">
                ...
                </a>) : (<div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center">
                  <Linkedin className="w-5 h-5 text-gray-400"/>
                </div>)}
              
              {/* Always show Email icon - conditionally active */}
              {member.email ? (<a href={`mailto:${member.email}`} className="group">
...
const TeamMemberCard = ({ member, onOpenModal }) => {
    const [imageLoading, setImageLoading] = useState(true);
    return (<div className="group h-full">
        <div className="relative rounded-xl overflow-hidden transition-all duration-300 ease-out h-full shadow-sm hover:shadow-md">
          ...
          <div className="relative bg-gray-100 overflow-hidden h-full">
            {/* Loading Skeleton */}
            {imageLoading && (<div className="absolute inset-0 bg-gray-200 animate-pulse z-[1]"/>)}

            {/* Image Container */}
            <div className="relative aspect-[3/4] w-full">
              <img src={member.photo_url} alt={member.name} className={`h-full w-full object-cover object-center transition-transform duration-700 ease-out ${imageLoading ? 'opacity-0' : 'opacity-100 group-hover:scale-110 group-hover:rotate-1'}`} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" onLoad={() => setImageLoading(false)} onError={() => setImageLoading(false)}/>
            ...
            <div className="absolute top-0 right-0 z-10">
              <div className="bg-white p-2 rounded-bl-xl relative">
                <button onClick={onOpenModal} aria-label="View team member details" className="w-8 h-8 bg-primary-orange rounded-full flex items-center justify-center transition-all duration-300 ease-out hover:bg-primary-green">
...
const FilterButton = ({ label, active, onClick }) => (<button onClick={onClick} className={`px-5 py-2 rounded-full border text-sm font-medium transition-all duration-300 ${active
...
const normalizeTeamTypeName = (name) => {
    return name?.trim().toLowerCase() || '';
};
const TeamPage = () => {
    const [activeFilter, setActiveFilter] = useState('advisory board'); // Changed default to 'advisory board'
    ...
    const getFilterButtonLabel = (typeName) => {
        // Capitalize the first letter of each word
        return typeName.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');
    };
    return (<main className="bg-background min-h-screen">
      {/* Hero Section with Header */}
      <div className="relative h-[500px]" style={{
      ...
      <div className="py-24">
        <Container>
          ...
          {errorMessage && (<div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-6" role="alert">
            ...
            <div className="lg:w-[280px] flex-shrink-0">
              <div className="lg:sticky lg:top-24">
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-4">
                  ...
                  {Array.isArray(teamTypes) && teamTypes.map((type) => (<FilterButton key={type.id} label={getFilterButtonLabel(type.name)} active={normalizeTeamTypeName(activeFilter) === normalizeTeamTypeName(type.name)} onClick={() => setActiveFilter(type.name)}/>))}
                </div>
              </div>
            </div>
        ...
        <div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
                    {filteredMembers.map((member) => (<TeamMemberCard key={member.id} member={member} onOpenModal={() => setSelectedMember(member)}/>))}
                  </div>
      ...
      {/* Modal */}
      {selectedMember && (<TeamMemberModal member={selectedMember} isOpen={!!selectedMember} onClose={() => setSelectedMember(null)}/>)}
