...
import { Search, Filter, ArrowRight, MapPin, User, ChevronRight, ChevronLeft } from 'lucide-react';
...
const ProjectCard = ({ project, getFeatureImage, getCategoryName }) => {
    const truncateDescription = (text) => {
        if (!text)
            return "A sustainable project working with local communities to improve agriculture systems.";
    ...
    return (<Link href={`projects/${project.id}`} className="block group">
          ...
          <div className="relative">
            ...
            <div className="relative aspect-[5/3] w-full overflow-hidden">
              <div className="absolute inset-0 w-full h-full bg-gray-100 flex items-center justify-center">
                <img src={getFeatureImage(project)} alt={project.name} className="w-full h-full object-contain transition-transform duration-700 group-hover:scale-105"/>
              </div>
...
const CategoryButton = ({ name, icon, count, isActive, onClick }) => {
    return (<button onClick={onClick} className={`flex items-center justify-between px-4 py-3 rounded-lg w-full transition-all duration-300 ${isActive
            ? 'bg-green-700 text-white shadow-md'
            : 'bg-white border border-gray-200 hover:bg-gray-50 text-gray-700'}`}>
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isActive ? 'bg-white' : 'bg-green-100'} text-green-700`}>
          {icon}
        </div>
...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
    return (<div className="flex justify-center items-center mt-8 space-x-2">
      <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className={`p-2 rounded-full border ${currentPage === 1 ? 'text-gray-400 border-gray-200' : 'text-green-700 border-green-600 hover:bg-green-50'}`}>
        <ChevronLeft className="w-5 h-5"/>
      </button>
      
      {[...Array(totalPages)].map((_, index) => (<button key={index} onClick={() => onPageChange(index + 1)} className={`w-10 h-10 rounded-full ${currentPage === index + 1
                ? 'bg-green-700 text-white'
                : 'text-gray-700 border border-gray-200 hover:bg-gray-50'}`}>
          {index + 1}
        </button>))}
      
      <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className={`p-2 rounded-full border ${currentPage === totalPages ? 'text-gray-400 border-gray-200' : 'text-green-700 border-green-600 hover:bg-green-50'}`}>
...
const ProjectsPage = () => {
    ...
    const getFeatureImage = (project) => {
        if (project.media && project.media.items && project.media.items.length > 0) {
            ...
            const featureImage = project.media.items.find((item) => item.tag === 'feature' || item.cover === true);
            if (featureImage && featureImage.url) {
                return featureImage.url;
            }
            // If no feature, try description
            const descriptionImage = project.media.items.find((item) => item.tag === 'description');
            ...
            const otherImage = project.media.items.find((item) => item.tag === 'others' || item.tag === 'other');
      ...
      {!loading && (<>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-1/4 space-y-3">
              ...
              {/* Category buttons */}
              {Object.entries(categories).reduce((unique, [id, name]) => {
            ...
            category && (<div key={category.id} className="category-button">
            ...
            <div className="lg:w-3/4">
              {projects.length === 0 ? (<div className="text-center py-12 bg-gray-50 rounded-lg">
