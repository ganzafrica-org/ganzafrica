...
import { ArrowLeft, Calendar, MapPin, Tag, Clock, CheckCircle, FileText, ImageIcon, Film, AlertCircle, Target, Award, Building } from 'lucide-react';
...
const ProjectDetailsPage = () => {
        ...
        const fetchProjectData = async () => {
            try {
                ...
                if (!projectData.goals)
                    projectData.goals = { items: [] };
                if (!projectData.outcomes)
                    projectData.outcomes = { items: [] };
                if (!projectData.media)
                    projectData.media = { items: [] };
    ...
    const getStatusBadge = (status) => {
        ...
        switch (status.toLowerCase()) {
            case 'completed':
                return (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 whitespace-nowrap">
            <Award className="w-3 h-3 mr-1 flex-shrink-0"/>
            <span className="truncate">Completed</span>
          </span>);
            case 'planned':
                return (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 whitespace-nowrap">
            <Clock className="w-3 h-3 mr-1 flex-shrink-0"/>
            <span className="truncate">Planned</span>
          </span>);
            case 'active':
                return (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 whitespace-nowrap">
    ...
    const renderMediaItem = (media) => {
        return (<div key={media.id} className="relative aspect-video rounded-lg overflow-hidden">
        {media.type === 'image' ? (<img src={getValidImageSrc(media.url)} alt={media.title || 'Project media'} className="object-cover w-full h-full" onError={(e) => {
                    e.currentTarget.src = '/images/news/maize.avif';
                }}/>) : (<div className="relative w-full h-full">
            <video src={media.url} controls className="w-full h-full object-cover" onError={(e) => {
        ...
        {media.caption && (<div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white p-2 text-sm">
    ...
    const galleryMedia = project.media && project.media.items ?
        ...
        [];
    return (<div className="min-h-screen bg-white dark:bg-gray-900 overflow-x-hidden">
      {/* Hero Section */}
      <div className="relative h-[50vh] sm:h-[70vh] md:h-[80vh] lg:h-[90vh] w-full overflow-hidden mt-16">
        {featuredMedia ? (<div className="relative w-full h-full">
            <img src={getValidImageSrc(featuredMedia.url)} alt={project.name} className="object-cover w-full h-full" onError={(e) => {
                e.currentTarget.src = '/images/news/maize.avif';
            }}/>
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent"/>
          </div>) : (<div className="w-full h-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
            <div className="text-center">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="container mx-auto px-4 pb-20">
            <div className="max-w-4xl">
              <Link href="/projects" className="inline-flex items-center text-white/80 hover:text-white transition-all duration-300 mb-12 group">
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-20 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-12">
          ...
          <div className="lg:col-span-2 space-y-16 w-full overflow-hidden">
            ...
            {/* Goals */}
            {project.goals && project.goals.items && project.goals.items.length > 0 && (<div className="space-y-8">
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="h-0.5 w-12 bg-primary-green flex-shrink-0"></div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white break-words">Project Goals</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {project.goals.items.map((goal) => (<div key={goal.id} className="p-8 bg-gray-50 dark:bg-gray-800 rounded-2xl transition-all hover:bg-gray-100 dark:hover:bg-gray-700">
                      <div className="flex items-center gap-4 mb-4">
                        {goal.completed ? (<div className="w-12 h-12 rounded-full bg-green-100 dark:bg-green-900/50 flex items-center justify-center">
                            <CheckCircle className="w-6 h-6 text-green-600 dark:text-green-400"/>
                          </div>) : (<div className="w-12 h-12 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
            ...
            {/* Outcomes */}
            {project.outcomes && project.outcomes.items && project.outcomes.items.length > 0 && (<div className="space-y-8">
                <div className="flex items-center gap-4 flex-wrap">
                ...
                <div className="grid grid-cols-1 gap-8">
                  {project.outcomes.items.map((outcome) => (<div key={outcome.id} className={`p-8 rounded-2xl transition-all duration-300 hover:shadow-lg ${outcome.status === 'achieved'
                    ...
                    : outcome.status === 'in_progress' || outcome.status === 'in-progress'
                      ...
                      <div className="flex items-start gap-6">
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${outcome.status === 'achieved'
                    ? 'bg-[#045F3C]/10 dark:bg-[#045F3C]/20'
                    : outcome.status === 'in_progress' || outcome.status === 'in-progress'
                        ? 'bg-[#FDB022]/10 dark:bg-[#FDB022]/20'
                        : 'bg-[#045F3C]/10 dark:bg-[#045F3C]/20'}`}>
                          <Award className={`w-8 h-8 ${outcome.status === 'achieved'
                    ...
                    : outcome.status === 'in_progress' || outcome.status === 'in-progress'
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-4 mb-4">
                            ...
                            <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-sm font-medium ${outcome.status === 'achieved'
                    ...
                    (outcome.status === 'in_progress' || outcome.status === 'in-progress') ? 'In Progress' : 'Planned'}
                          ...
                          {/* Metrics */}
                          {outcome.metrics && outcome.metrics.length > 0 && (<div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-3">
                              {outcome.metrics.map((metric, index) => (<div key={index} className="flex items-start gap-2">
            ...
            {galleryMedia.length > 0 && (<div className="space-y-6">
                ...
                <div className="relative group">
                  ...
                  <div className="relative aspect-[16/9] rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-800 group">
                    {galleryMedia[activeImageIndex]?.type === 'image' ? (<img src={getValidImageSrc(galleryMedia[activeImageIndex]?.url)} alt={galleryMedia[activeImageIndex]?.title || 'Featured media'} className="object-cover w-full h-full transition-transform duration-700 group-hover:scale-105" onError={(e) => {
                    e.currentTarget.src = '/images/news/maize.avif';
                }}/>) : galleryMedia[activeImageIndex]?.type === 'video' ? (<div className="w-full h-full">
                        <video src={galleryMedia[activeImageIndex]?.url} controls className="w-full h-full object-cover" onError={(e) => {
                    // Add fallback UI for video errors
                    const parent = e.currentTarget.parentElement;
                    if (parent) {
                        parent.innerHTML = `
                      ...
                      </div>) : (<div className="w-full h-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                        <ImageIcon className="w-16 h-16 text-gray-400"/>
                      </div>)}
                    {galleryMedia[activeImageIndex]?.type === 'video' && (<div className="absolute inset-0 flex items-center justify-center bg-black/20">
                        <button className="w-20 h-20 text-primary-orange rounded-full flex items-center justify-center hover:bg-yellow-500 transition-all duration-300 hover:scale-110 transform hover:shadow-xl" aria-label="Play video" title="Play video">
                          <div className="w-0 h-0 border-t-[15px] border-t-transparent border-l-[25px] border-l-white border-b-[15px] border-b-transparent ml-2"></div>
                        </button>
                      </div>)}
                    {galleryMedia[activeImageIndex]?.caption && (<div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/50 to-transparent translate-y-4 transition-transform duration-300 group-hover:translate-y-0">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 sm:gap-4 mt-4">
                    {galleryMedia.map((item, index) => (<div key={item.id} className={`relative aspect-video rounded-lg overflow-hidden cursor-pointer group ${index === activeImageIndex ? 'ring-2 ring-yellow-400' : ''}`} onClick={() => setActiveImageIndex(index)} role="button" tabIndex={0} onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                        setActiveImageIndex(index);
                    }
                }} aria-label={`View ${item.title || `image ${index + 1}`}`}>
                        {item.type === 'image' ? (<img src={getValidImageSrc(item.url)} alt={item.title || `Gallery item ${index + 1}`} className="object-cover w-full h-full transition-transform duration-500 group-hover:scale-110" onError={(e) => {
                        e.currentTarget.src = '/images/news/maize.avif';
                    }}/>) : item.type === 'video' ? (<div className="relative w-full h-full bg-gray-200 dark:bg-gray-700">
                        ...
                        {item.caption && (<div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                  ...
                  {/* Navigation Arrows */}
                  {galleryMedia.length > 1 && (<div className="absolute top-1/2 -translate-y-1/2 left-4 right-4 flex justify-between pointer-events-none">
                      <button onClick={handlePrevImage} className="w-12 h-12 rounded-full text-primary-orange flex items-center justify-center hover:bg-yellow-500 transition-all duration-300 transform hover:scale-110 hover:shadow-lg pointer-events-auto opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0" aria-label="Previous image" title="View previous image">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M15 18l-6-6 6-6"/>
                        </svg>
                      </button>
                      <button onClick={handleNextImage} className="w-12 h-12 rounded-full text-primary-orange flex items-center justify-center hover:bg-yellow-500 transition-all duration-300 transform hover:scale-110 hover:shadow-lg pointer-events-auto opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0" aria-label="Next image" title="View next image">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {galleryMedia.length > 1 && (<div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2">
                      {galleryMedia.map((_, index) => (<button key={index} onClick={() => setActiveImageIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === activeImageIndex
                        ? 'w-8 text-primary-orange'
                        : 'bg-white/50 hover:bg-white/80'}`} aria-label={`Go to image ${index + 1}`} title={`View image ${index + 1}`}/>))}
                    </div>)}
                </div>
              </div>)}
          </div>
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="space-y-6">
              ...
              {project.members && Array.isArray(project.members) && project.members.length > 0 ? (<div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-6">
                  <div className="space-y-6">
                ...
                return (<div key={memberKey} className="flex items-center gap-3">
                          {photoUrl ? (<div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0">
                              <img src={getValidImageSrc(photoUrl)} alt={memberName} className="w-full h-full object-cover" onError={(e) => {
                            const parent = e.currentTarget.parentElement;
                            if (parent) {
                                parent.innerHTML = `
                ...
                </div>) : (<div className="p-6 bg-gray-50 dark:bg-gray-800 rounded-2xl text-center">
            ...
            {project.partners && project.partners.length > 0 && (<div className="space-y-6">
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="h-0.5 w-12 bg-primary-green flex-shrink-0"></div>
                ...
                return (<div key={partnerKey} className="flex flex-col items-center text-center gap-3 p-4">
