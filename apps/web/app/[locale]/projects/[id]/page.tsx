...
const ProjectDetailsPage = ({ params }: { params: { id: string } }) => {
  ...
  const renderMediaItem = (media: Media) => {
    return (
      <div key={media.id} className="bg-white p-4 border rounded-lg shadow-sm">
        <div className="flex justify-between items-start mb-2">
          <h3 className="font-medium text-gray-900">{media.title}</h3>
          <span className="text-xs px-2 py-1 bg-gray-100 rounded-full">{media.tag}</span>
        </div>
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            ...
            <ul className="space-y-2">
              <li className="flex items-center text-sm">
                ...
                <span className="font-medium">{getCategoryName(project.category_id)}</span>
              </li>
              <li className="flex items-center text-sm">
        ...
        <div className="w-full md:w-3/4">
          ...
          {activeTab === 'details' && (
            <div className="bg-white p-6 rounded-lg border border-gray-200">
              ...
              {/* Basic info section */}
              <div className="mb-8">
                <h2 className="text-xl font-bold mb-4">Timeline Information</h2>
