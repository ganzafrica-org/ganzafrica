...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    try {
        ...
        const [newsResponse, projectsResponse, opportunitiesResponse, blogsResponse, eventsResponse] = await Promise.allSettled([
            ...
            news.forEach((article: { title: string; publish_date?: string; updated_at?: string }) => {
                if (article.title) {
                    ...
                    routes.push({
                        ...
                        lastModified: article.updated_at ? new Date(article.updated_at) : article.publish_date ? new Date(article.publish_date) : new Date(),
            ...
            projects.forEach((project: { id: string; updated_at?: string; created_at?: string }) => {
                if (project.id) {
                    routes.push({
                        ...
                        lastModified: project.updated_at ? new Date(project.updated_at) : project.created_at ? new Date(project.created_at) : new Date(),
        ...
        if (opportunitiesResponse.status === "fulfilled") {
            const opportunities = opportunitiesResponse.value.data?.opportunities || opportunitiesResponse.value.data || [];
            opportunities.forEach((opportunity: { id: string; updated_at?: string; created_at?: string }) => {
                if (opportunity.id) {
                    routes.push({
                        url: `${baseUrl}/opportunities/${opportunity.id}`,
                        lastModified: opportunity.updated_at ? new Date(opportunity.updated_at) : opportunity.created_at ? new Date(opportunity.created_at) : new Date(),
                        changeFrequency: "weekly",
                        priority: 0.8,
                    });
                }
        ...
        if (blogsResponse.status === "fulfilled") {
            ...
            blogs.forEach((blog: { slug?: string; title?: string; updated_at?: string; created_at?: string }) => {
