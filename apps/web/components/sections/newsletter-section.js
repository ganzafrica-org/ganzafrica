...
export default function NewsletterSection({ locale, dict, }) {
        ...
        <div className="max-w-2xl mx-auto text-center">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
            <div className="flex-grow">
              <input type="email" placeholder="Enter email address" className="bg-white w-full px-4 py-3 rounded-md border border-gray focus:outline-none focus:ring-2 focus:ring-primary-green" value={email} onChange={(e) => setEmail(e.target.value)} required/>
            </div>
            <button type="submit" className={`px-6 py-3 bg-primary-orange text-primary-green font-medium rounded-md hover:bg-primary-orange/90 transition-colors ${isSubmitting ? "opacity-70 cursor-not-allowed" : ""}`} disabled={isSubmitting}>
              {isSubmitting ? "Subscribing..." : "Subscribe"}
            </button>
          </form>

          {/* Status messages */}
          {submitStatus === "success" && (<p className="mt-4 text-primary-green">
              Thank you for subscribing to our newsletter!
            </p>)}
          {submitStatus === "error" && (<p className="mt-4 text-red-500">
