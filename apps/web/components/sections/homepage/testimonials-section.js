...
export default function TestimonialsSection({ locale, dict }) {
    ...
    if (loading && testimonials.length === 0) {
        return (<section className="py-16 md:py-24 bg-secondary-green/5 relative overflow-hidden">
                        ...
                        <div className="relative">
                            ...
                            <div className="hidden md:block absolute left-0 top-1/2 -translate-y-1/2">
                                <div className="relative w-40 h-96">
                                    {[1, 2, 3, 4].map((_, index) => (<div key={`skeleton-left-${index}`} className="absolute animate-pulse" style={{
                    top: `${index * 18}%`,
                    left: '50%',
                }}>
                            ...
                            <div className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2">
                                <div className="relative w-40 h-96">
                                    {[1, 2, 3, 4].map((_, index) => (<div key={`skeleton-right-${index}`} className="absolute animate-pulse" style={{
                    top: `${index * 18}%`,
                    right: '50%',
                }}>
                                            <div className="w-16 h-16 rounded-full bg-gray-200 border-2 border-white shadow-md"></div>
                        ...
                        <div className="md:hidden flex justify-center mt-8 gap-3">
                            {[1, 2, 3, 4].map((_, index) => (<div key={`skeleton-nav-${index}`} className="w-10 h-10 rounded-full bg-gray-200 animate-pulse"></div>))}
                        </div>
                    </div>
                </div>
            </section>);
    ...
    return (<section className="py-16 md:py-24 bg-secondary-green/5 relative overflow-hidden">
            <div className="container mx-auto px-4">
                <div className="text-center mb-16">
                    <DecoratedHeading firstText={dict?.testimonials?.heading_first || "Our"} secondText={dict?.testimonials?.heading_second || "Testimonials"} className="mx-auto"/>
                </div>
            ...
            return (<div key={`left-${testimonial.id}`} className={cn("absolute transition-all duration-500 cursor-pointer", isActive
                    ...
                    transition: "all 0.3s ease-in-out"
                                            ...
                                            <Avatar className={cn("border-2 border-white shadow-md", isActive ? "w-20 h-20" : "w-16 h-16" // Increased sizes
                )}>
                        ...
                        <div className="flex flex-col items-center justify-center text-center px-4 md:px-20">
                            ...
                            <div className="relative h-56 w-full max-w-xs mb-8 perspective-container">
                                {testimonials.map((testimonial, index) => (<div key={`center-${testimonial.id}`} className={cn("absolute top-0 left-0 right-0 mx-auto transition-all duration-700 ease-in-out", index === activeIndex ? "opacity-100 transform-none" :
                index < activeIndex ? "opacity-0 -translate-y-full rotate-x-70" : "opacity-0 translate-y-full rotate-x-negative-70")}>
                            ...
                            <div className="relative min-h-[180px] md:min-h-[150px] w-full perspective-container">
                                {testimonials.map((testimonial, index) => (<div key={`quote-${testimonial.id}`} className={cn("absolute w-full px-4 transition-all duration-700 ease-in-out", index === activeIndex ? "opacity-100 transform-none" :
                index < activeIndex ? "opacity-0 -translate-y-full rotate-x-70" : "opacity-0 translate-y-full rotate-x-negative-70")}>
                                        <div className="relative">
            ...
            const radius = 20; // pixels from center
            return (<div key={`right-${testimonial.id}`} className={cn("absolute transition-all duration-500 cursor-pointer", isActive
                    ? "ring-4 ring-primary-green/70 ring-offset-2 scale-110"
                    : "opacity-80 hover:opacity-100 hover:scale-105")} style={{
                    ...
                    <div className="md:hidden flex justify-center mt-8 gap-3">
                        {testimonials.map((testimonial, index) => (<button key={`nav-${index}`} onClick={() => handleAvatarClick(index)} className={cn("transition-all rounded-full overflow-hidden border-2", index === activeIndex
                ? "scale-125 border-primary-green"
                : "scale-100 border-transparent opacity-70 hover:opacity-90")} aria-label={`Go to testimonial ${index + 1}`}>
