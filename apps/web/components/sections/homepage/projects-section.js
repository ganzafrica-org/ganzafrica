...
export default function ProjectsSection({ locale, dict }) {
    ...
    const getSlidePosition = (index) => {
        ...
        if (diff === 0)
            return 'position-3'; // center
        if (diff === 1 || diff === -4)
            return 'position-4'; // right middle
        if (diff === 2 || diff === -3)
            return 'position-5'; // far right
        if (diff === -1 || diff === 4)
            return 'position-2'; // left middle
        if (diff === -2 || diff === 3)
            return 'position-1'; // far left
        return 'position-none'; // hidden
    };
    const getFeatureImageUrl = (project) => {
    ...
    if (loading && projects.length === 0) {
        return (<section className="py-16 md:py-24 bg-secondary-green/10 relative overflow-hidden">
    ...
    return (<section className="py-16 md:py-24 bg-secondary-green/10 relative overflow-hidden">
            <div className="container mx-auto px-4">
                <div className="text-center mb-16">
                    <DecoratedHeading firstText={dict?.projects?.heading_first ?? "Our"} secondText={dict?.projects?.heading_second ?? "Projects"} className="mx-auto"/>
                </div>
                ...
                <div className="relative mb-12">
                    <div className="h-[400px] md:h-[500px] relative perspective-1000">
                        <div className="slider-content relative h-full w-full flex justify-center items-center">
                            {projects.map((project, idx) => (<div key={project.id} className={`
                                        absolute rounded-2xl overflow-hidden transition-all duration-500
                                        ${getSlidePosition(idx)}
                                    `}>
                                    <div className="relative w-full h-full">
                                        <img src={getFeatureImageUrl(project)} alt={project.name} className="w-full h-full object-cover"/>

                                        <div className={`absolute inset-0 ${idx === activeIndex ? 'bg-black/20' : 'bg-black/80'}`}/>


                                        {/* View project link - Only visible for center slide */}
                                        {idx === activeIndex && (<Link href={getProjectUrl(project.id)} className="absolute top-4 right-4 bg-white text-primary-green flex items-center gap-1 text-sm font-medium px-3 py-1.5 rounded-full hover:bg-primary-green hover:text-white transition-colors">
                    ...
                    <div className="flex justify-center items-center gap-3 mt-8">
                        <div className="w-32 md:w-40 h-1.5 bg-primary-orange rounded-full overflow-hidden relative">
                            <div className="h-full bg-secondary-green absolute left-0 top-0 transition-all duration-500 ease-in-out w-10" style={{ left: `${(activeIndex * 100) / Math.max(projects.length, 1)}%` }}/>
                        </div>

                        <button onClick={prevSlide} className="p-2 rounded-full bg-white hover:bg-secondary-green transition-colors" aria-label="Previous project">
                            <ChevronLeft className="w-6 h-6 text-primary-orange"/>
                        </button>

                        <button onClick={nextSlide} className="p-2 rounded-full bg-white hover:bg-secondary-green transition-colors" aria-label="Next project">
                ...
                <div className="text-center mt-12">
                    <Link href={`/${locale}/projects`} className="inline-flex items-center gap-2 bg-primary-green hover:bg-primary-green/90 text-white py-3 px-6 rounded-lg transition-colors">
