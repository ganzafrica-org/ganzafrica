...
export default function GanzAfricaUniqueSection({ locale, dict, }) {
    const [videoPlaying, setVideoPlaying] = useState(false);
    const videoRef = useRef(null);
    // Get section content from dictionary with fallbacks
    const sectionTitle = dict?.unique?.title || "Three Key Elements that make GanzAfrica Unique";
    // Define custom SVG icons for each element
    const DataIcon = () => (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    const ImplementationIcon = () => (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M22 12H18L15 21L9 3L6 12H2" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>);
    const PublicSectorIcon = () => (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    const keyElements = [
        {
            ...
            description: dict?.unique?.elements?.data?.description ||
        ...
        },
        {
            ...
            description: dict?.unique?.elements?.implementation?.description ||
        ...
        {
            title: dict?.unique?.elements?.public_sector?.title || "The Public Sector",
            description: dict?.unique?.elements?.public_sector?.description ||
    ...
    return (<section className="py-16 md:py-24 bg-[#e3f1ed]">
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <div>
              ...
              <div className="relative rounded-2xl overflow-hidden aspect-video bg-black/10">
                <video ref={videoRef} src="/videos/farmers-in-field.mp4" // Your video file path
     poster="/images/famer-feild.png" // Optional: keep the thumbnail as a poster
     className="w-full h-full object-cover" onEnded={() => setVideoPlaying(false)}/>

                {/* Play/Pause button overlay */}
                <div className="absolute inset-0 flex items-center justify-center">
                  <button onClick={handleVideoPlayback} className="w-16 h-16 md:w-20 md:h-20 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 hover:bg-white/30" aria-label={videoPlaying ? "Pause video" : "Play video"}>
                    {videoPlaying ? (<PauseCircle className="w-12 h-12 md:w-16 md:h-16 text-white drop-shadow-lg"/>) : (<PlayCircle className="w-12 h-12 md:w-16 md:h-16 text-white drop-shadow-lg"/>)}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              {keyElements.map((element, index) => (<div key={element.title} className="bg-white rounded-lg overflow-hidden shadow-sm flex flex-col md:flex-row">
                    {/* Colored sidebar */}
                    <div className={`w-full md:w-2 h-2 md:h-auto bg-${element.color}`}></div>
