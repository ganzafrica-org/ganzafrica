  ...
  const getImageUrl = (newsItem: NewsItem): string => {
    try {
      ...
      if (newsItem.media?.items?.length) {
        ...
        const coverImage = newsItem.media.items.find(
          (item) => item.cover === true && item.type === "image" && item.url,
        );
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map((item) => (
              <div
                key={item}
                className="bg-white rounded-3xl overflow-hidden shadow-lg animate-pulse perspective-wrapper"
              >
  ...
  if (!loading && newsItems.length === 0) {
    return (
      <section className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <DecoratedHeading firstText="Latest" secondText="News" className="mx-auto" />
          </div>
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-800 mb-2">
              <TranslatableText>No News Available</TranslatableText>
            </h3>
              ...
              >
                  ...
                  >
                    ...
                    {newsItem.category_name && (
                      <span className="absolute top-3 left-3 bg-primary-green text-white text-xs font-semibold px-2.5 py-1 rounded-sm z-10">
                        <TranslatableText>{newsItem.category_name}</TranslatableText>
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="p-5 bg-white">
                  ...
                  <div className="flex items-center gap-2 text-gray-500 text-sm mb-3">
                    ...
                    <span>
                      <TranslatableText>{formatDate(newsItem.publish_date)}</TranslatableText>
                    </span>
                  </div>

                  {/* Title */}
                  <h3 className="text-base font-bold text-gray-900 mb-2 line-clamp-2">
                    <TranslatableText>{newsItem.title}</TranslatableText>
                  </h3>
                  ...
                  >
                    <span className="border-b border-transparent group-hover:border-current transition-all duration-300">
                      <TranslatableText>{`Read more about ${newsItem.title}`}</TranslatableText>
                    </span>
                    <svg
        ...
        {newsItems.length > 0 && (
          <div className="text-center mt-12">
            <Link
              ...
              onClick={() =>
                trackEvent("view_all_news_click", {
                  source_page: "homepage",
                })
              }
            >
