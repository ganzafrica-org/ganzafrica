...
export default function NewsSection({ locale, dict }) {
    ...
    const getImageUrl = (newsItem) => {
        try {
            ...
            if (newsItem.media?.items?.length) {
                ...
                const coverImage = newsItem.media.items.find(item => item.cover === true && item.type === 'image' && item.url);
                if (coverImage && coverImage.url) {
                    return coverImage.url;
                }
                // If no cover image, try the first image
                const firstImage = newsItem.media.items.find(item => item.type === 'image' && item.url);
    ...
    if (loading) {
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map((item) => (<div key={item} className="bg-white rounded-3xl overflow-hidden shadow-lg animate-pulse perspective-wrapper">
    ...
    if (!loading && newsItems.length === 0) {
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <DecoratedHeading firstText={dict?.news?.heading_first ?? "Latest"} secondText={dict?.news?.heading_second ?? "News"} className="mx-auto"/>
          </div>
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {newsItems.map((newsItem) => {
            ...
            return (<motion.div key={newsItem.id} className="news-card perspective-wrapper" whileHover={{ y: -5, transition: { duration: 0.2 } }}>
                {/* Image with perspective effect */}
                <div className="perspective-element">
                  <div className="perspective-image" style={{
                    backgroundImage: `url(${getImageUrl(newsItem)})`,
                    backgroundColor: '#117B34', // Backup color if image fails
                }}>
                    {/* Category badge */}
                    {newsItem.category_name && (<span className="absolute top-3 left-3 bg-primary-green text-white text-xs font-semibold px-2.5 py-1 rounded-full z-10">
                ...
                <div className="p-5 bg-white">
                  ...
                  <Link href={`/${locale}/newsroom/${slug}`} className="view-news-button">
        ...
        {newsItems.length > 0 && (<div className="text-center mt-12">
            <Link href={`/${locale}/newsroom`} className={cn("inline-flex items-center gap-2", "bg-primary-green hover:bg-primary-green/90", "text-white py-2.5 px-6 rounded-lg", "transition-all duration-300 hover:shadow-lg", "text-sm font-medium")}>
