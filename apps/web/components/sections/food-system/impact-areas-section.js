...
const ClimateInitiativesMapSection = () => {
    ...
    const isRwandaDistrict = (location) => {
        if (!location)
            return false;
        ...
        return rwandaDistricts.some(district => locationLower === district ||
    ...
    const getCoordinatesForLocation = (location) => {
        ...
        if (locationLower in knownLocations) {
            const locationData = knownLocations[locationLower];
        ...
        if (locationLower.includes(',')) {
            ...
            if (firstLocation in knownLocations) {
                const locationData = knownLocations[firstLocation];
                return {
                    lat: locationData.lat,
                    lng: locationData.lng,
                    mapUrl: 'mapUrl' in locationData ? locationData.mapUrl : undefined
    ...
    const generateMapUrl = (coordinates, zoomLevel = 12) => {
        if (!coordinates)
            return null;
    ...
    const getProjectImage = (project) => {
        ...
        const featureImage = project.media.items.find(item => item.tag === 'feature' && item.url && item.type === 'image');
        // If no feature image, use the first available image
        const anyImage = project.media.items.find(item => item.url && item.type === 'image');
    ...
    const projectsAtSameLocation = currentProject ?
        projectLocations.filter(p => p.mapCoordinates && currentProject.mapCoordinates &&
    ...
    return (<section className="py-16 bg-white">
      <Container>
        <motion.div variants={fadeIn} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="text-center mb-10">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <motion.div className="grid grid-cols-3 gap-6 max-w-xl mx-auto mb-8" variants={statsVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
    ...
    <motion.div className="relative inline-block w-full sm:w-56" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6 }}>
      <select value={selectedCountry} onChange={handleCountryChange} className="appearance-none bg-white border border-green-700 rounded-md py-2 pl-3 pr-10 w-full text-gray-700 focus:outline-none">
    ...
    <button className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-3 rounded-md text-sm font-medium transition-colors">
        ...
        {/* Map visualization */}
        <motion.div className="relative h-96 w-full rounded-lg overflow-hidden shadow-md border-2 border-gray-300" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} ref={mapRef}>
          {isLoading ? (<div className="flex items-center justify-center h-full bg-gray-100">
              <p className="text-gray-500">Loading map...</p>
            </div>) : filteredLocations.length === 0 ? (<div className="flex flex-col items-center justify-center h-full bg-gray-100">
              ...
              <iframe ref={mapIframeRef} src={currentProject && currentProject.mapCoordinates && currentProject.mapCoordinates.mapUrl
                ...
                : mapCenter && mapCenter.mapUrl
                    ...
                    : selectedCountry === 'rwanda'
                        ...
                        : selectedCountry === 'burkina faso'
                            ...
                            : "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d31397.814232798383!2d20.053565!3d0.084886!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x1779fe8521916c39%3A0x2caec1cf01ad37f!2sAfrica!5e0!3m2!1sen!2sus!4v1681732186562!5m2!1sen!2sus"} width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`GanzAfrica Projects Map - ${selectedCountry.charAt(0).toUpperCase() + selectedCountry.slice(1)}`}></iframe>

              {/* Map controls for zoom */}
              <div className="absolute top-3 right-3 flex flex-col space-y-2 map-control z-10">
                <button className="bg-white rounded-full w-8 h-8 shadow-md flex items-center justify-center hover:bg-gray-100" onClick={handleZoomIn}>
                  <Plus className="w-5 h-5 text-gray-700"/>
                </button>
                <button className="bg-white rounded-full w-8 h-8 shadow-md flex items-center justify-center hover:bg-gray-100" onClick={handleZoomOut}>
              ...
              {filteredLocations.map((location, locationIndex) => {
                ...
                if (location.mapCoordinates) {
                    ...
                    if (multipleProjectsAtLocation[clusterKey]) {
                        ...
                        if (clusterIndex === -1)
                            clusterIndex = locationIndex % clusterTotal;
                    }
                }
                ...
                return (<div key={location.id} className="absolute z-10" style={{
                        left: `${position.x}px`,
                        top: `${position.y}px`,
                    }}>
                    {/* Project marker */}
                    <div className="relative cursor-pointer" onClick={() => handleProjectClick(location.id)} onMouseEnter={() => handleProjectMouseEnter(location.id)} onMouseLeave={handleProjectMouseLeave}>
                      {/* Marker with profile image */}
                      <div className={`rounded-full overflow-hidden transform -translate-x-1/2 -translate-y-1/2 transition-transform duration-200 ${isSelected ? 'scale-110 z-30' : 'hover:scale-105'}`} style={{
                        width: '40px',
                        height: '40px',
                        border: `3px solid ${isSelected ? '#F59E0B' : '#047857'}`,
                        boxShadow: '0 2px 10px rgba(0,0,0,0.2)'
                    }}>
                        <img src={location.image} alt={location.title} className="w-full h-full object-cover" onError={(e) => {
                        const target = e.target;
                        target.onerror = null;
                      ...
                      {(isSelected || isHovered) && (<div className="absolute whitespace-nowrap text-center mt-1 text-xs font-medium bg-white px-2 py-1 rounded-md shadow-sm -translate-x-1/2" style={{ top: '100%', left: '50%' }}>
                          {location.location}
                        </div>)}
                      
                      {/* Cluster indicator - show for first marker in cluster */}
                      {showClusterCount && clusterTotal > 1 && (<div className="absolute bg-orange-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center -translate-x-1/2 -translate-y-1/2" style={{ top: '-8px', right: '-8px' }} onClick={(e) => handleClusterClick(clusterKey, e)}>
                          {clusterTotal}
                        </div>)}
                    </div>

                    {/* Project card - show on selection or hover */}
                    {(isSelected || isHovered) && (<div className="absolute bg-white rounded-lg shadow-lg overflow-hidden z-20 project-card w-52" style={{
                            top: '-105px',
                            left: '-110px',
                        ...
                        <button className="absolute top-2 right-2 bg-white rounded-full p-1 shadow-md hover:bg-gray-100 z-30" onClick={handleCloseProject}>
                          ...
                          <div className="p-3">
                            <h3 className="font-bold text-green-700 text-sm mb-1 line-clamp-1">
                              ...
                              <>
                                ...
                                <Link href={`/${locale}/projects/${location.projectId}`} className="text-xs text-yellow-600 hover:text-yellow-800 font-medium">
                                  Learn more
                                </Link>
                              </>
                          </div>
              ...
              {selectedProject && projectsAtSameLocation.length > 1 && (<div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-20 projects-carousel">
                  <div className="bg-white rounded-lg shadow-lg p-2 flex space-x-2 items-center">
                    ...
                    {projectsAtSameLocation.map((project) => (<div key={project.id} className="block group cursor-pointer" onClick={() => handleProjectClick(project.id)}>
                        <img src={project.image} alt={project.title} className="w-8 h-8 rounded-full object-cover border-2 border-transparent group-hover:border-yellow-500" onError={(e) => {
                        const target = e.target;
                        target.onerror = null;
