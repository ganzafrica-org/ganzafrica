...
function parseLocation(locationString) {
    if (!locationString)
        return { country: 'rwanda', location: 'Rwanda' };
...
const FoodSystemsMapSection = () => {
    ...
    const filteredLocations = projectLocations.filter((location) => location.country === selectedCountry);
    ...
    return (<section className="py-16 bg-white">
      <Container>
        ...
        {loading ? (<div className="w-full h-64 flex items-center justify-center">
          ...
          </div>) : (<div className="flex flex-col items-center mb-10">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-8">
              ...
              <motion.button className="bg-primary-orange hover:bg-yellow-600 text-white px-4 py-3 rounded-md text-sm font-medium transition-colors w-full sm:w-auto" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6 }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                Highlights of our work
              </motion.button>
            </div>

            {/* Stats grid */}
            <motion.div className="grid grid-cols-2 sm:grid-cols-4 gap-6 max-w-xl mx-auto mb-8" variants={statsVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
              {stats.map((stat, index) => (<motion.div key={index} className="text-center" variants={statItemVariants}>
        ...
        <motion.div className="relative h-96 w-full rounded-lg overflow-hidden shadow-md border-2 border-gray-300" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} ref={mapRef}>
          ...
          <iframe ref={mapIframeRef} src={currentProject
            ...
            : filteredLocations.length > 0
                ...
                : selectedCountry === 'burkina'
                    ...
                    : "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d63817.18087378733!2d30.019363028729005!3d-1.944098787600761!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x19dca42968f6b901%3A0xfba4f422b2a13a89!2sKigali%2C%20Rwanda!5e0!3m2!1sen!2sus!4v1712031042989!5m2!1sen!2sus"} width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="GanzAfrica Location"></iframe>
            ...
            return (<div key={location.id} className="absolute z-10" style={{
                    ...
                    {/* Project marker */}
                    <div className="relative cursor-pointer" onClick={() => handleProjectClick(location.id)}>
                      {/* Marker with profile image - enhanced visibility */}
                      <div className={`rounded-full overflow-hidden transform -translate-x-1/2 -translate-y-1/2 transition-transform duration-200 ${isSelected ? "scale-110" : ""}`} style={{
                    width: "60px",
                    height: "60px",
                    ...
                    zIndex: isSelected ? 20 : 10,
                      ...
                      <div className={`absolute whitespace-nowrap text-center mt-1 text-xs font-medium bg-white px-2 py-1 rounded-md shadow-md -translate-x-1/2 transition-all duration-300 ${isSelected ? 'text-green-700 font-bold' : 'text-gray-700'}`} style={{
                    top: "100%",
                    left: "50%",
                    ...
                    {isSelected && (<div className={`absolute bg-white rounded-lg shadow-xl overflow-hidden transition-all duration-300 z-30 ${isExpanded ? "w-72" : "w-60"}`} style={{
                        top: "-120px",
                        left: "-110px",
                        ...
                        <div className="relative">
                          {/* Project image */}
                          <div className={`relative ${isExpanded ? "h-36" : "h-28"}`}>
                            <img src={location.image} alt={location.title} className="w-full h-full object-cover"/>
                            ...
                            <button className="absolute bottom-2 right-2 bg-white rounded-full p-1.5 shadow-md hover:bg-gray-100 transition-all duration-200" onClick={(e) => handleExpandClick(location.id, e)}>
                              {isExpanded ? (<X className="w-5 h-5 text-gray-700"/>) : (<Info className="w-5 h-5 text-green-700"/>)}
                            </button>
                          </div>
                          ...
                          <div className="p-3">
                            ...
                            {isExpanded ? (<>
                                ...
                                <a href={location.url} className="text-xs text-yellow-600 hover:text-yellow-800 font-medium inline-flex items-center bg-yellow-50 rounded-md px-2 py-1 transition-colors hover:bg-yellow-100">
                                  Learn more
                                ...
                                <a href={location.url} className="text-xs text-green-700 hover:text-green-800 font-medium inline-flex items-center">
                                  View details
                                  <ChevronRight className="ml-1 w-3 h-3"/>
                                </a>
                              </>)}
