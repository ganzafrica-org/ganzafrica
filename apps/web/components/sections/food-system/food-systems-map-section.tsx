...
function generateMapLocations(projects) {
  ...
  const projectsWithBasicData = projects.map((project, index) => {
    ...
    const image =
      project.media?.items?.find((item) => item.tag === "feature")?.url ||
      "/images/news/maize.avif";
...
const FoodSystemsMapSection = () => {
      ...
      try {
        ...
        const mockProjects = [
          ...
          },
          {
            ...
            description:
              "Helping farmers adapt to changing climate conditions with resilient crop varieties and practices.",
  ...
  const handleProjectClick = (projectId) => {
    ...
    if (selectedProject === projectId) {
      ...
      if (mapIframeRef.current && project) {
        ...
        mapIframeRef.current.src = `${project.mapUrl}&center=${project.mapCoordinates.lat},${project.mapCoordinates.lng}&zoom=${zoom}&markers=color:red%7Clabel:${markerLabel}%7C${project.mapCoordinates.lat},${project.mapCoordinates.lng}`;
      }
    }
  };
  ...
  const getMarkerPosition = (position) => {
    if (!position || !mapDimensions.width || !mapDimensions.height) {
      ...
      const defaultX =
        Math.random() * 0.8 * (mapDimensions.width || 600) + 0.1 * (mapDimensions.width || 600);
      const defaultY =
        Math.random() * 0.8 * (mapDimensions.height || 400) + 0.1 * (mapDimensions.height || 400);
      ...
      <Container>
        ...
        >
          ...
          <p className="text-gray-600 max-w-3xl mx-auto">
            GanzAfrica operates in multiple countries, equipping young professionals with the skills
            and opportunities to drive meaningful change in Africa's agri-food systems.
          </p>
        </motion.div>
                        ...
                        {isExpanded ? (
                          <>
                            ...
                            <div className="flex items-start mb-2">
                              ...
                              <p className="text-xs text-gray-600 ml-1">{location.address}</p>
                            </div>
