    ...
    return (<section className="py-16 bg-white">
      <Container>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          ...
          {/* Partners logos */}
          <motion.div className="grid grid-cols-2 sm:grid-cols-3 gap-6" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
            {partnerLogos.map((logo, index) => (<motion.div key={index} className="flex items-center justify-center h-20" variants={logoVariants} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Image src={logo.src} alt={logo.name} width={150} height={80} className="object-contain max-h-full"/>
              </motion.div>))}
          </motion.div>
        </div>
      </Container>
