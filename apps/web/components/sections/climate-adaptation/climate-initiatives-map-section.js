...
const ClimateInitiavesMapSection = () => {
    ...
    useEffect(() => {
        ...
        apiClient.get('/teams')
            ...
            const fellowsCount = teamsArray.filter(member => member.team_type === 'fellow').length;
            setStatsData(prev => ({
                ...prev,
    ...
    return (<section className="py-16 bg-white">
            <Container>
                ...
                <div className="flex flex-col items-center mb-10">
                    ...
                    <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-8">
                        ...
                        <motion.button className="bg-primary-orange hover:bg-yellow-600 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors w-full sm:w-auto" initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6 }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            ...
            filteredLocations[0]?.mapUrl} width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="GanzAfrica Location"></iframe>
            ...
            return (<div key={location.id} className="absolute z-10" style={{
                    ...
                    boxShadow: '0 2px 10px rgba(0,0,0,0.2)'
                                    ...
                                    {/* Location label */}
                                    {isSelected && (<div className="absolute whitespace-nowrap text-center mt-1 text-xs font-medium bg-white px-2 py-1 rounded-md shadow-sm -translate-x-1/2" style={{ top: '100%', left: '50%' }}>
                                            {location.location}
                                        </div>)}
                                </div>

                                {/* Project card */}
                                {isSelected && (<div className={`absolute bg-white rounded-lg shadow-lg overflow-hidden transition-all duration-300 z-20 ${isExpanded ? 'w-64' : 'w-52'}`} style={{
                        top: '-105px',
                        left: '-110px',
                        transform: isExpanded ? 'scale(1.1)' : 'scale(1)'
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className={`relative ${isExpanded ? 'h-32' : 'h-24'}`}>
                                                ...
                                                <button className="absolute bottom-2 right-2 bg-white rounded-full p-1 shadow-sm hover:bg-gray-100" onClick={(e) => handleExpandClick(location.id, e)}>
                                                    {isExpanded ? (<X className="w-4 h-4 text-gray-600"/>) : (<Info className="w-4 h-4 text-gray-600"/>)}
                                                </button>
                                            </div>
                                            ...
                                            <div className="p-3">
                                                ...
                                                {isExpanded ? (<>
                                                        ...
                                                        <a href={location.url} className="text-xs text-yellow-600 hover:text-yellow-800 font-medium inline-flex items-center">
                                                            Learn more
                                                        ...
                                                        <a href={location.url} className="text-xs text-yellow-600 hover:text-yellow-800 font-medium">
                                                            View details
                                                        </a>
                                                    </>)}
                                            </div>
