...
const ProjectCard: React.FC<any> = ({ project, getFeatureImage, getCategoryName }) => {
  ...
  return (
    <Link
      ...
      aria-label="Know more on this project"
        ...
        <div className="relative w-full overflow-hidden">
          <div className="relative">
            <div className="absolute top-3 left-3 z-10 bg-white py-1 px-2 rounded-full text-xs font-medium shadow-md transform transition-transform duration-300 group-hover:scale-110">
              <TranslatableText>{project.status || "Active"}</TranslatableText>
            </div>
                ...
                <img
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src =
                      "/api/placeholder/400/250?text=Image+Not+Available";
                  }}
                />
              </div>
        ...
        <div className="px-4 py-3 flex justify-between items-center bg-white">
          <div className="flex items-center text-green-700 group-hover:text-green-600 transition-colors">
            ...
            <span className="font-medium text-sm">
              <TranslatableText>{project.location || "Rwanda"}</TranslatableText>
            </span>
          </div>
...
const CategoryButton: React.FC<any> = ({ name, icon, count, isActive, onClick }) => {
  return (
    ...
    >
      ...
      <span
        className={`text-sm ${isActive ? "bg-white text-[#FFB800]" : "bg-gray-200 text-gray-700"} px-2 py-0.5 rounded-full`}
      >
        {count}
      </span>
    </button>
...
export default function ProjectsPageContent() {
  ...
  const getFeatureImage = (project: any): string => {
    if (project.media && project.media.items && project.media.items.length > 0) {
      const featureImage = project.media.items.find(
        (item: any) => item.tag === "feature" || item.cover === true,
      );
      ...
      const otherImage = project.media.items.find(
        (item: any) => item.tag === "others" || item.tag === "other",
      );
  ...
  const handlePageChange = (pageNumber: number): void => {
    ...
    if (projectsGridRef.current) projectsGridRef.current.scrollIntoView({ behavior: "smooth" });
  };
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="lg:w-1/4 flex lg:flex-col gap-3 overflow-x-auto pb-4 lg:pb-0 scrollbar-hide lg:border-none border-b lg:space-y-3">
                ...
                {Object.entries(categories)
                  .reduce<any[]>((unique, [id, name]) => {
                    if (!unique.some((item) => item.name === name)) {
                      unique.push({ id: parseInt(id), name, count: categoryCounts[name] || 0 });
                    }
                    return unique;
                  }, [])
                  .map(
                ...
                {projects.length === 0 ? (
                  <div className="text-center py-12 bg-gray-50 rounded-lg">
                    <p className="text-gray-500">
                      <TranslatableText>No projects found matching your criteria</TranslatableText>
                    </p>
                  </div>
                ) : (
                  <>
