        ...
        <div className="p-6 flex items-start gap-6">
          ...
          <div className="flex-1 pt-2">
            <h2 className="text-[24px] font-bold text-[#111827] leading-tight mb-2">
              <TranslatableText>{member.name}</TranslatableText>
            </h2>
            <p className="text-[15px] text-[#6B7280] tracking-wide">
              <TranslatableText>{member.position}</TranslatableText>
            </p>
          </div>
        </div>
          ...
          <div className="py-5 border-t border-[#E5E7EB]">
            ...
            >
              <p className="whitespace-normal break-words">
                <TranslatableText>{member.bio}</TranslatableText>
              </p>
            </div>
          </div>
        ...
        <div className="relative bg-gray-100 overflow-hidden h-full">
          ...
          <div className="absolute top-0 right-0 z-10">
            <div className="bg-white p-2 rounded-bl-xl relative">
              ...
              >
                <ArrowUpRightIcon
                  className="w-3.5 h-3.5 text-white transform transition-transform group-hover:rotate-45"
                  strokeWidth={2}
                />
              </button>
            </div>
...
const TeamPage = (): JSX.Element => {
    ...
    const fetchTeams = async () => {
      try {
        ...
        const response = await apiClient.get("/teams", {
          params: { sort_by: "sort_order", sort_order: "asc" },
        });
  ...
  const filteredMembers = teamMembers.filter((member) => {
    ...
    if (
      (teamTypeName.includes("advisor") || teamTypeName.includes("board")) &&
    ...
    if (
      (teamTypeName.includes("alumni") || teamTypeName.includes("alum")) &&
      ...
      >
        ...
        <div className="relative z-10 flex items-center justify-center h-full text-center mt-[-50px]">
          <div className="space-y-8">
            <div className="text-6xl font-bold text-primary-orange">
              {getDynamicHeaderText(activeFilter)}
            </div>
                  ...
                  {Array.isArray(teamTypes) &&
                    teamTypes.map((type) => (
                      <FilterButton
                        ...
                        active={
                          normalizeTeamTypeName(activeFilter) === normalizeTeamTypeName(type.name)
                        }
