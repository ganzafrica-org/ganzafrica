...
export default function Navigation({ locale, dict, isHomePage = false, }) {
    ...
    const getHeaderBgClass = () => {
        if (isHomePage) {
            return cn("fixed top-0 z-50 min-w-full transition-all duration-500", isScrolled ? "bg-white shadow-sm backdrop-blur-sm" : "");
        }
        else {
    ...
    return (<header ref={navRef} className={getHeaderBgClass()}>
              ...
              <NavigationMenuList>
                  ...
                  <NavigationMenuContent>
                    <ul className="grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]">
                      <li className="row-span-3">
                        <NavigationMenuLink asChild>
                          <a className="flex h-full w-full select-none flex-col justify-end rounded-md bg-gradient-to-b from-muted/50 to-muted p-6 no-underline outline-none focus:shadow-md" href={`/${locale}/about/who-we-are`}>
                      ...
                      {aboutItems.map((item) => (<ListItem key={item.href} href={`/${locale}${item.href}`} title={dict?.about?.[item.title.toLowerCase().replace(/ /g, "_")] || item.title}>
                          {item.description}
                        </ListItem>))}
                    </ul>
                  </NavigationMenuContent>
                ...
                <NavigationMenuItem>
                  <Link href={`/${locale}/our-approach`} passHref>
                    <NavigationMenuLink className={`${getNavItemColor()} block px-4 py-2 text-base font-medium hover:text-accent-foreground`}>
                      {dict?.navigation?.our_approach || "Our Approach"}
                    </NavigationMenuLink>
                  </Link>
                </NavigationMenuItem>
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent>
                    <ul className="grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]">
                      <li className="row-span-3">
                        <NavigationMenuLink asChild>
                          <a className="flex h-full w-full select-none flex-col justify-end rounded-md bg-gradient-to-b from-muted/50 to-muted p-6 no-underline outline-none focus:shadow-md" href={`/${locale}/projects`}>
                            <div className="mb-2 mt-4 text-lg font-medium">
                             Our Projects
                           </div>
                            <p className="text-sm leading-tight text-muted-foreground">
                      ...
                      {programsItems.map((item) => (<ListItem key={item.href} href={`/${locale}${item.href}`} title={item.title}>
                          {item.description}
                        </ListItem>))}
                    </ul>
                  </NavigationMenuContent>
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent>
                    <ul className="grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]">
                      <li className="row-span-3">
                        <NavigationMenuLink asChild>
                          <a className="flex h-full w-full select-none flex-col justify-end rounded-md bg-gradient-to-b from-muted/50 to-muted p-6 no-underline outline-none focus:shadow-md" href={`/${locale}/newsroom`}>
                            <div className="mb-2 mt-4 text-lg font-medium">
                              News & Updates
                            </div>
                            <p className="text-sm leading-tight text-muted-foreground">
                      ...
                      {newsItems.map((item) => (<ListItem key={item.href} href={`/${locale}${item.href}`} title={item.title}>
                          {item.description}
                        </ListItem>))}
                    </ul>
                  </NavigationMenuContent>
          ...
          <div className="min-h-full p-4 w-auto flex items-center">
            <div className="flex items-center gap-2">
              ...
              <div className="md:hidden">
                <Button variant="ghost" size="icon" className="text-black hover:bg-[#F5F5F5] transition-colors" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label={isMobileMenuOpen ? "Close menu" : "Open menu"}>
