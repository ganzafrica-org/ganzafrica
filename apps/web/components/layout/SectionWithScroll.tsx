...
export default function SectionWithScrollAnimation({ 
  ...
  return (
    <div 
      ref={sectionRef}
      className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12 overflow-hidden"
    >
      <motion.div 
        className={`p-4 ${bgColor}  h-96 w-full ${contentOrder}`}
      ...
      >
        <div className="flex flex-col h-full w-full">
          <motion.div 
            className={`${accentColor} text-white w-10 h-10 flex items-center justify-center text-lg font-bold mb-2`}
          ...
          <div className={contentClass}>
            <p className="text-black font-regular-small text-sm">
              {text}
            </p>
          </div>
        </div>
      ...
      <motion.div 
        className={`p-4 h-96 w-full ${imageOrder}`}
