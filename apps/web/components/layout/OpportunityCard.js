...
export function OpportunityCard({ title, status, description, icon, color, requirements, type = "fellowship", duration, location, employmentType, startDate, endDate, }) {
    // Format dates
    ...
    return (<div className="bg-white rounded-xl shadow-lg overflow-hidden border-2 border-[#045f3c] transform hover:scale-[1.02] transition-transform duration-300 group">
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge variant={status === "Open" ? "default" : "secondary"} className={`${status === "Open"
        ...
        <div className="space-y-2 mb-6">
          ...
          {requirements.map((req, index) => (<div key={index} className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-[#F8B712]"/>
              <span className="text-sm">{req}</span>
            </div>))}
        </div>

        <Button className={`w-full group ${status === "Open"
            ? "bg-[#045f3c] hover:bg-[#F8B712] hover:text-black"
            : "bg-[#045f3c]/10 text-[#045f3c] cursor-not-allowed"}`} disabled={status === "Closed"}>
