...
export default function LanguageSwitcher(): JSX.Element {
  ...
  const renderFlag = (lang: (typeof SUPPORTED_LANGUAGES)[number], size: number = 20) => {
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
          ...
          {isTranslating ? (
            ...
            <>
              ...
              {isTranslated && targetLanguage !== sourceLanguage && (
                <span className="ml-1 w-2 h-2 bg-[#00A651] rounded-full" title="Translated" />
              )}
            </>
          )}
        </Button>
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        {SUPPORTED_LANGUAGES.map((language) => (
          ...
          >
            {renderFlag(language)}
            <span className={targetLanguage === language.code ? "font-semibold" : ""}>
              {language.name}
            </span>
        ...
        {error && (
          <>
            <DropdownMenuSeparator />
            <div className="px-2 py-1.5 text-xs text-red-500">Translation unavailable</div>
          </>
        )}
      </DropdownMenuContent>
    </DropdownMenu>
