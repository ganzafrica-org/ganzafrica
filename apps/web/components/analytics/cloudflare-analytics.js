...
export function CloudflareAnalytics({ token }) {
    // Only include the beacon if a token is provided
    if (!token)
        return null;
    return (<>
      {/* Cloudflare Web Analytics */}
      <Script id="cloudflare-analytics" strategy="afterInteractive" defer src="https://static.cloudflareinsights.com/beacon.min.js" data-cf-beacon={`{"token": "${token}"}`}/>
    </>);
}
