...
export const helpdeskService = {
    ...
    async getTickets(params?: { search?: string; status?: string; category?: string; priority?: string; page?: number; limit?: number }) {
        const response = await httpClient.get<PaginatedResponse<HelpdeskTicketPayload>>("/helpdesk", { params })
    ...
    async updateTicket(ticketId: string, payload: HelpdeskTicketPayload) {
        const response = await httpClient.patch<HelpdeskTicketPayload>(`/helpdesk/${ticketId}`, payload)
        return response.data
    },
}
