...
export const assetsService = {
    ...
    async updateAsset(id: string, payload: UpdateAssetRequest, imageFiles?: File[]): Promise<Asset> {
        if (imageFiles && imageFiles.length > 0) {
            ...
            imageFiles.forEach((file) => form.append("images", file))
            const response = await httpClient.patch<{ success: boolean; data: Asset }>(`${BASE}/${id}`, form, {
                headers: {"Content-Type": "multipart/form-data"},
            })
            return response.data.data
        }
        const response = await httpClient.patch<{ success: boolean; data: Asset }>(`${BASE}/${id}`, payload)
        return response.data.data
    },
    ...
    async getCategories(): Promise<AssetCategory[]> {
        const result = await httpClient.get<{ success: boolean; data: AssetCategory[] }>(`${BASE}/categories`)
        return result.data.data
    },

    async getCategoryById(id: string): Promise<AssetCategory> {
        const result = await httpClient.get<{ success: boolean; data: AssetCategory }>(`${BASE}/categories/${id}`)
        return result.data.data
    },

    async createCategory(payload: CreateCategoryRequest): Promise<AssetCategory> {
        const result = await httpClient.post<{ success: boolean; data: AssetCategory }>(`${BASE}/categories`, payload)
        return result.data.data
    },
    ...
    async listMaintenance(assetId?: string): Promise<AssetMaintenance[]> {
        const result = await httpClient.get<{ success: boolean; data: AssetMaintenance[] }>(`${BASE}/maintenance`, {
            params: {assetId},
        })
        return result.data.data
    },
    ...
    async updateMaintenance(id: string, payload: UpdateMaintenanceRequest): Promise<AssetMaintenance> {
        const result = await httpClient.patch<{
            success: boolean;
            data: AssetMaintenance
