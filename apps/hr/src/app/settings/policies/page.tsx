...
export default function PoliciesPage() {
  ...
  const filteredPolicies = policyList.filter((policy) => {
    ...
    const matchesFeature = selectedFeature === "all" || (policy.feature ?? "—") === selectedFeature;
                  ...
                  <tbody>
                    {filteredPolicies.map((policy, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 text-sm text-gray-700">{policy.feature ?? "—"}</td>
                        <td className="px-4 py-4">
                          <Badge
                            className={`${getStatusBadgeClass(policy.status)} border-0 rounded-full px-3 py-1`}
                          >
