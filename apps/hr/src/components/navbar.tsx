...
import { Bell, Search, User, Menu, Settings, LogOut, Sun, Moon, Calendar, Settings as SettingsIcon, HelpCircle, Eye, X, Home, Users, Briefcase, FileText, UserCircle, Clock } from "lucide-react"
        ...
        >
                ...
                <div className="flex items-center gap-3 lg:gap-9">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="relative h-10 w-10 rounded-full bg-white/10 text-white hover:bg-brand-accent border-none">
                                <Bell className="h-5 w-5" />
                                <span className="absolute top-2.5 right-2.5 h-2 w-2 bg-red-500 rounded-full border-[#134e48]"></span>
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-80 mt-2 p-0 overflow-hidden bg-white dark:bg-slate-900 border-white/10 dark:border-slate-800 rounded-lg">
                            <DropdownMenuLabel className="p-4 flex items-center justify-between">
                                <span className="font-bold text-base">Notifications</span>
                                <Badge variant="secondary" className="bg-[#00df82]/10 text-brand-accent border-none">3 New</Badge>
                            </DropdownMenuLabel>
                                ...
                                <div className="p-4 border-b border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer transition-colors">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground mt-0.5">Agricultural Specialist position</p>
                                ...
                                <div className="p-4 border-b border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer transition-colors">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground mt-0.5">Marie Claire Nsengimana</p>
                                ...
                                <div className="p-4 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer transition-colors">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground mt-0.5">Version 2.4 is now live</p>
                            ...
                            <DropdownMenuSeparator className="m-0 bg-white/10" />
                            <Button variant="ghost" className="w-full rounded-none h-12 text-sm text-[#00df82] hover:text-[#00df82] hover:bg-slate-50 dark:hover:bg-white/5">
                    ...
                    <Link href="/settings">
                        <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full bg-white/10 text-white hover:bg-brand-accent border-none">
                    ...
                    {isAuthenticated && user ? (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="p-0 h-10 w-10 rounded-full hover:opacity-80 border-none">
                                    <Avatar className="h-10 w-10 border-none border-white/20">
                                        <AvatarImage src={user.avatarUrl} />
                                        <AvatarFallback className="bg-brand-accent">{getInitials(user.name)}</AvatarFallback>
                                    </Avatar>
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-56 mt-2 rounded-lg">
                                <DropdownMenuLabel className="font-normal">
                                    <div className="flex flex-col space-y-1">
                                        ...
                                        <Badge variant="outline" className="w-fit text-[10px] px-1 py-0 mt-1">{user.role}</Badge>
                                    </div>
                                </DropdownMenuLabel>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem>
                        ...
                        <Link href="/auth/login">
                            <Button variant="ghost" className="text-white/80 hover:text-white hover:bg-white/10 rounded-full px-4">
                                Sign In
                            </Button>
                        </Link>
                    )}
            ...
            <div className="md:hidden flex h-16 w-full items-center justify-between px-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
                <div className="flex items-center gap-3">
                    ...
                    <Sheet>
                        <SheetTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full text-slate-600 dark:text-slate-300">
                                <Menu className="h-6 w-6" />
                            </Button>
                        </SheetTrigger>
                        <SheetContent side="left" className="w-[300px] sm:w-[350px] p-0 border-none bg-brand-dark text-white">
                ...
                <div className="flex items-center gap-1">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="relative h-9 w-9 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-none">
                                <Bell className="h-5 w-5" />
                        ...
                        <DropdownMenuContent align="end" className="w-80 mt-2 p-0 overflow-hidden bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-lg">
                            ...
                            <div className="max-h-80 overflow-y-auto">
                                ...
                                <div className="p-4 border-b border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer transition-colors">
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium">New application received</p>
                                            <p className="text-xs text-muted-foreground mt-0.5">Agricultural Specialist</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <Link href="/settings">
                        <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                            <SettingsIcon className="h-5 w-5" />
                        </Button>
                    </Link>
                    {isAuthenticated && user ? (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="p-0 h-8 w-8 ml-1 rounded-full hover:opacity-80 border-none">
                        ...
                        <Link href="/auth/login">
                            <Button variant="ghost" className="text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full px-4 text-xs h-8">
                                Sign In
                            </Button>
                        </Link>
                    )}
                    ...
                    {navItems.map((item) => (
                        <Link
                            ...
                            className={`text-sm font-medium transition-colors px-3 py-1 rounded-full ${
                                isActive(item.href) ? "bg-[#00df82] text-[#134e48]" : "text-white/70 hover:text-white"
                            }`}
                        >
                            {item.label}
                        </Link>
