              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge variant="outline" className="capitalize">
                    {asset.category?.parent_name ? `${asset.category.parent_name} › ` : ""}
                    {asset.category?.name}
                  </Badge>
                </div>
            ...
            {asset.specs && asset.specs.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                {asset.specs.map((spec: any) => {
                  const label =
                    (asset.category?.spec_schema ?? []).find((f: any) => f.key === spec.spec_key)
                      ?.label ?? spec.spec_key;
                  const unit = (asset.category?.spec_schema ?? []).find(
                    (f: any) => f.key === spec.spec_key,
                  )?.unit;
                  return (
                    <div
                      key={spec.spec_key}
                      className="flex justify-between p-2.5 bg-white bg-opacity-60 rounded-md border border-purple-100"
                    >
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                {asset.images.map((image: any) => (
                  <div key={image.id} className="relative group flex-shrink-0">
                    ...
                    />
                    {image.isPrimary && (
                      <Badge className="absolute top-1 left-1 bg-blue-600 text-[10px] px-1.5 h-4">
                        Primary
                      </Badge>
                    )}
                  </div>
