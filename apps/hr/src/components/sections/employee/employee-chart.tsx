...
import { INITIAL_EMPLOYEES, initialEmployee } from "@/data/employee-data";

export default function DepartmentChartPage() {
  const [employees, setEmployees] = useState<initialEmployee[]>(INITIAL_EMPLOYEES);
  const [draggedEmployeeId, setDraggedEmployeeId] = useState<string | null>(null);
  const [dropTargetId, setDropTargetId] = useState<string | null>(null);
  const [selectedEmployee, setSelectedEmployee] = useState<initialEmployee | null>(null);
  const [isAddSheetOpen, setIsAddSheetOpen] = useState(false);
  const [zoom, setZoom] = useState(1);
  const [newEmployee, setNewEmployee] = useState<Partial<initialEmployee>>({
    name: "",
    role: "",
    parentId: "",
    email: "",
  ...
  const isDescendant = useCallback(
    (parentId: string, childId: string, allEmployees: initialEmployee[]) => {
  ...
  const handleDrop = (e: React.DragEvent, targetId: string) => {
    ...
    setEmployees((prev) =>
      prev.map((emp) => (emp.id === draggedEmployeeId ? { ...emp, parentId: targetId } : emp)),
    );
  };

  const renderCard = (employee: initialEmployee) => (
    <div
      key={employee.id}
      draggable
        ...
        <CardContent className="p-4 relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-[14px] font-bold text-slate-700 truncate leading-tight">
                {employee.name}
              </p>
              {employee.role === "Team" && (
                <p className="text-[11px] text-slate-400 font-medium">Department Team</p>
              )}
            </div>
          </div>
              ...
              <div className="flex items-center gap-3 text-slate-600">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold">
                    {employees.find((e) => e.id === selectedEmployee.parentId)?.name ||
                      "None (CEO)"}
                  </p>
                </div>
              </div>
