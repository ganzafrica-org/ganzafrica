...
export default function BoardPage(): React.JSX.Element {
  ...
  useEffect(() => {
    const loadData = async () => {
      await Promise.all([loadAllTasks(), loadTeamMembers()]);
    };
    loadData();
  ...
  const filteredTasks = useMemo(() => {
    let filtered =
      selectedMember === "all"
        ? tasks
        : tasks.filter((task) =>
            (task.assignees || []).some((a) => a != null && a.toString() === selectedMember),
          );
  ...
  const handlePriorityDrop = async (e: React.DragEvent, priority: "high" | "medium" | "low") => {
    ...
    try {
      ...
      if (task) {
        ...
        await taskApi.updateTaskWithFallback(
          parseInt(id),
          taskData,
          isCurrentUserAdminOrManagerRole(),
        );
      }
    } catch (err: any) {
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-600 mt-1">
                    This view shows only tasks created by users with management roles (manager,
                    admin, etc.). Tasks created by regular users will not appear here.
                  </p>
                </div>
              </div>
            </div>
        ...
        <TaskModal
          ...
          onChange={async (updatedTask: Task) => {
            try {
              ...
              // Use fallback method for task updates
              const response = await taskApi.updateTaskWithFallback(
      ...
      <TaskModal
        ...
        task={
          isCreatingTask
            ? {
                ...
                priority:
                  activeTab === "management" ? (creatingTaskPriority as any) : ("medium" as const),
                dueDate: undefined,
                labels: [],
                assignees: [],
                comments: [],
        ...
        onChange={async (updatedTask: Task) => {
          ...
          if (updatedTask.teamId) {
            const selectedTeamProject = taskTeamProjects.find(
              (p: any) => p.team_id === parseInt(updatedTask.teamId!),
            );
          ...
          try {
            ...
            try {
              ...
              const fallbackTask = {
                ...response.task,
                assignees:
                  response.task.assignees?.map((a: any) =>
                    a.user_id ? a.user_id.toString() : "",
                  ) || [],
              ...
              {members.map((member) => (
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <div className="text-xs" style={{ color: "#6b7280" }}>
                      {member.email} • {tasks.filter((t) => t.assignees.includes(member.id)).length}{" "}
                      tasks assigned
                    </div>
                  </div>
