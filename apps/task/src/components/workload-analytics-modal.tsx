      ...
      >
        ...
        <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between" style={{ borderColor: '#e5e7eb', borderRadius: '7px 7px 0 0' }}>
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h3 className="text-lg font-semibold" style={{ color: '#1f2937' }}>Workload Analytics</h3>
              <p className="text-sm" style={{ color: '#6b7280' }}>{member.name}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{ borderColor: member.color }}></div>
            </div>
          ) : weeklyAnalytics ? (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-4 rounded-lg border" style={{ borderColor: '#e5e7eb' }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium" style={{ color: '#6b7280' }}>Occupancy</span>
                  </div>
                ...
                <div className="p-4 rounded-lg border" style={{ borderColor: '#e5e7eb' }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium" style={{ color: '#6b7280' }}>Hours</span>
                  </div>
                  <p className="text-2xl font-bold" style={{ color: member.color }}>
                ...
                <div className="p-4 rounded-lg border" style={{ borderColor: '#e5e7eb' }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium" style={{ color: '#6b7280' }}>Tasks</span>
                  </div>
                  <p className="text-2xl font-bold" style={{ color: member.color }}>
              ...
              {/* Daily Breakdown */}
              <div>
                <h4 className="text-md font-semibold mb-4" style={{ color: '#1f2937' }}>Daily Breakdown</h4>
                <div className="space-y-2">
                  {weeklyAnalytics.days.map((day, index) => (
                    <div
                      key={index}
                      className={`p-3 rounded-lg border flex items-center justify-between ${
                        day.isOccupied ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs" style={{ color: '#6b7280' }}>
                            {day.isOccupied ? `${day.hoursWorked}h • ${day.tasksCount} tasks` : 'No tasks'}
                          </p>
                        </div>
                      </div>
              ...
              {/* Occupancy Chart */}
              <div>
                <h4 className="text-md font-semibold mb-4" style={{ color: '#1f2937' }}>Weekly Occupancy</h4>
            ...
            <div className="text-center py-8">
              <p className="text-sm" style={{ color: '#6b7280' }}>No data available for this period</p>
            </div>
          )}
        </div>
