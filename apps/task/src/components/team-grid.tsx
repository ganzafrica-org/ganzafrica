          ...
          <div key={m.id} className="rounded-2xl border border-black/5 bg-white p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="ml-auto text-sm">
                ...
                <span className="font-semibold">{activeCounts[m.id] || 0}</span>
              </div>
            </div>
            <div className="mt-3 space-y-2">
              {visible.map((t) => (
                <div
                  key={t.id}
                  className="px-3 py-2 rounded-md bg-gradient-to-r from-indigo-50 to-purple-50 border border-black/5 text-sm"
                >
                  {t.title}
                </div>
              ))}
