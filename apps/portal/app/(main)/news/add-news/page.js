...
import { ArrowLeft, Plus, X, Upload, Image as ImageIcon, FileVideo, Check, AlertCircle, Loader, ChevronDown, Tag as TagIcon } from 'lucide-react';
...
const AddNewsPage = () => {
            ...
            if (mediaToRemove.cover && updatedItems.length > 0) {
                ...
                if (firstImageItem) {
                    return {
                        ...prev,
                        media: {
                            items: updatedItems.map(item => item.id === firstImageItem.id
                                ? { ...item, cover: true }
                                : item)
                        }
                    };
    ...
    const handleSubmit = async (e) => {
            ...
            if (!hasCover) {
                ...
                if (firstImage) {
                    setFormData(prev => ({
                        ...prev,
                        media: {
                            items: prev.media.items.map(item => item.id === firstImage.id
                                ? { ...item, cover: true }
                                : item)
                        }
                    }));
        ...
        try {
            // Prepare data for API with the media URLs
            const newsData = {
                ...formData,
                tags: selectedTags,
                // Note: media items already contain URLs pointing to our backend
            };
              ...
              <div className="border-2 border-dashed border-gray-300 p-6 rounded-md text-center mb-6 cursor-pointer" onClick={!newMedia.file ? triggerFileInput : undefined}>
                ...
                removeNewMedia();
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <input type="text" name="title" value={newMedia.title} onChange={handleMediaChange} className="w-full p-2 border border-gray-300 rounded-md" placeholder="Enter media title" onClick={(e) => e.stopPropagation()}/>
                      </div>
                      <div>
                        <label htmlFor="mediaCover" className="text-sm">
                          <input type="checkbox" id="mediaCover" name="cover" checked={newMedia.cover} onChange={handleMediaChange} onClick={(e) => e.stopPropagation()} className="mr-2"/>
                          Use as cover image
                        </label>
                      </div>
              ...
              {formData.media.items.length === 0 ? (<p className="text-gray-500 text-sm italic mb-4">No media added yet.</p>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  {formData.media.items.map(media => (<div key={media.id} className={`border rounded-md overflow-hidden ${selectedMedia && selectedMedia.id === media.id ? 'ring-2 ring-green-500' : ''}`}>
                      ...
                      <div className="h-32 bg-gray-100 flex items-center justify-center overflow-hidden cursor-pointer relative" onClick={() => selectMedia(media.id)}>
                        {media.type === 'image' ? (<img src={media.url} alt={media.title} className="object-cover w-full h-full" onError={(e) => {
                      ...
                      {selectedMedia && selectedMedia.id === media.id && (<div className="p-3 border-t flex justify-between">
                          ...
                          <button type="button" onClick={() => removeMedia(media.id)} className="text-xs bg-red-100 text-red-700 hover:bg-red-200 px-2 py-1 rounded-md flex items-center ml-auto">
