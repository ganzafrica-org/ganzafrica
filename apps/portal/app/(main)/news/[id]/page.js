...
import { ArrowLeft, Calendar, Clock, User, Tag as TagIcon, Eye, AlertCircle, Edit, Share2, FileVideo } from 'lucide-react';
...
const NewsDetailsPage = ({ params }) => {
    ...
    const getKeyLessons = (keyLessons) => {
        if (!keyLessons)
            return [];
    ...
    if (loading) {
        return (<div className="p-4 md:p-6 max-w-full mx-auto container flex items-center justify-center min-h-[80vh]">
    ...
    if (error) {
        return (<div className="p-4 md:p-6 max-w-full mx-auto container">
    ...
    return (<div className="p-4 md:p-6 max-w-full mx-auto container overflow-hidden">
      {/* Header section */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
        ...
        <div className="flex space-x-2 mt-2 sm:mt-0">
          <button onClick={handleShare} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200" title="Share">
            <Share2 className="w-5 h-5 text-gray-700"/>
          </button>
          <Link href={`/news/edit/${news.id}`} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200" title="Edit">
      ...
      <div className="md:hidden mb-4 border-b border-gray-200">
        <div className="flex space-x-4 overflow-x-auto pb-2">
          <button onClick={() => setActiveTab('article')} className={`px-3 py-2 whitespace-nowrap ${activeTab === 'article' ? 'text-green-700 border-b-2 border-green-700' : 'text-gray-600'}`}>
            Article
          </button>
          <button onClick={() => setActiveTab('info')} className={`px-3 py-2 whitespace-nowrap ${activeTab === 'info' ? 'text-green-700 border-b-2 border-green-700' : 'text-gray-600'}`}>
            Information
          </button>
          <button onClick={() => setActiveTab('media')} className={`px-3 py-2 whitespace-nowrap ${activeTab === 'media' ? 'text-green-700 border-b-2 border-green-700' : 'text-gray-600'}`}>
            Media
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6">
        ...
        <div className={`hidden md:block w-full md:w-1/4 bg-white p-4 rounded border border-gray-200 h-fit sticky top-4`}>
          <ul>
            <li className="mb-6">
              <button onClick={() => setActiveTab('article')} className={`w-full text-left flex items-start ${activeTab === 'article' ? 'text-green-700' : 'text-gray-700'}`}>
            ...
            <li className="mb-6">
              <button onClick={() => setActiveTab('info')} className={`w-full text-left flex items-start ${activeTab === 'info' ? 'text-green-700' : 'text-gray-700'}`}>
                <div className="flex-shrink-0 mt-1">
        ...
        <div className="w-full md:w-3/4">
          {/* Article tab */}
          {activeTab === 'article' && (<div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200 overflow-hidden">
              {/* Cover image */}
              {getCoverImage() && (<div className="mb-6">
                  <img src={getCoverImage() || ''} alt={news.title} className="w-full h-48 md:h-64 object-cover rounded-lg" onLoad={() => console.log(`Successfully loaded cover image: ${getCoverImage()}`)} onError={(e) => {
              ...
              <div className="mb-6 flex flex-wrap gap-4 text-sm text-gray-600">
                ...
                {news.publish_date && (<div className="flex items-center">
                ...
                {news.views !== undefined && (<div className="flex items-center">
              ...
              {/* Tags */}
              {news.tags && news.tags.length > 0 && (<div className="mb-6 overflow-x-auto">
                  <div className="flex flex-wrap gap-2">
                    {news.tags.map(tag => (<div key={tag.id} className="px-3 py-1 rounded-full bg-green-50 text-green-700 text-sm flex items-center whitespace-nowrap">
              ...
              {/* Summary */}
              {news.summary && (<div className="mb-6">
              ...
              {/* Key lessons */}
              {news.key_lessons && (<div className="mb-6">
                  <h2 className="text-xl font-bold mb-4">Key Takeaways</h2>
                  <div className="space-y-2">
                    {getKeyLessons(news.key_lessons).map((lesson, index) => (<div key={index} className="p-3 bg-gray-50 rounded-md border-l-4 border-green-700 break-words">
                        <p>{lesson}</p>
                      </div>))}
                  </div>
                </div>)}
          ...
          {/* Info tab */}
          {activeTab === 'info' && (<div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                {news.publish_date && (<div className="p-4 bg-gray-50 rounded-lg">
                    <div className="text-sm text-gray-500 mb-1">Published Date</div>
                    <div className="font-medium">{formatDate(news.publish_date)}</div>
                  </div>)}
                
                {news.views !== undefined && (<div className="p-4 bg-gray-50 rounded-lg">
              ...
              <div className="mb-6 p-4 bg-gray-50 rounded-lg overflow-x-auto">
                {news.tags && news.tags.length > 0 ? (<div className="flex flex-wrap gap-2">
                    {news.tags.map(tag => (<div key={tag.id} className="px-3 py-1 rounded-full bg-green-50 text-green-700 text-sm flex items-center whitespace-nowrap">
                        <TagIcon className="w-3 h-3 mr-1"/>
                        <span>{tag.name}</span>
                      </div>))}
                  </div>) : (<p className="text-gray-500 text-sm">No tags assigned</p>)}
              </div>
              ...
              {news.key_lessons && (<>
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg mb-6 overflow-hidden">
                    <ul className="list-disc pl-5 space-y-1 break-words">
                      {getKeyLessons(news.key_lessons).map((lesson, index) => (<li key={index}>{lesson}</li>))}
                    </ul>
                  </div>
                </>)}
            </div>)}
          ...
          {/* Media tab */}
          {activeTab === 'media' && (<div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200">
              <h2 className="text-xl font-bold mb-6">Media Files</h2>
              
              {news.media && news.media.items && news.media.items.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {news.media.items.map((media) => (<div key={media.id} className="border rounded-lg overflow-hidden">
                      {media.type === 'image' ? (<div className="h-48 bg-gray-100">
                          <img src={media.url} alt={media.title || 'Image'} className="w-full h-full object-cover" onLoad={() => console.log(`Successfully loaded image: ${media.url}`)} onError={(e) => {
                        ...
                        </div>) : media.type === 'video' ? (<div className="h-48 bg-gray-100 relative">
                          <div className="absolute inset-0 flex items-center justify-center">
                            {media.thumbnailUrl ? (<img src={media.thumbnailUrl} alt={media.title || 'Video thumbnail'} className="w-full h-full object-cover absolute inset-0" onLoad={() => console.log(`Successfully loaded video thumbnail: ${media.thumbnailUrl}`)} onError={(e) => {
                      ...
                      <div className="p-4 overflow-hidden">
                        ...
                        {media.cover && (<div className="mt-2">
                            <span className="px-2 py-1 text-xs bg-green-100 text-green-800 rounded-full">Cover image</span>
                          </div>)}
                        
                        <div className="mt-3">
                          <a href={media.url} target="_blank" rel="noopener noreferrer" className="text-green-700 hover:underline text-sm">
