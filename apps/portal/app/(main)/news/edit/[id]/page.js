...
import { ArrowLeft, Plus, X, Upload, Image as ImageIcon, FileVideo, Check, AlertCircle, Loader, ChevronDown, Tag as TagIcon, Link as LinkIcon } from 'lucide-react';
...
const EditNewsPage = ({ params }) => {
    ...
    if (error && !formData.title) {
        ...
        <div className="bg-red-50 border border-red-200 rounded-md p-4">
          ...
          <Link href="/news" className="mt-4 inline-flex items-center px-4 py-2 bg-red-100 text-red-700 rounded-md hover:bg-red-200">
            Return to News List
          </Link>
        </div>
      </div>);
    ...
    return (<div className="max-w-7xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md">
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-3/4">
              <div className="mb-6">
                ...
                <input type="text" name="title" value={formData.title} onChange={handleChange} className="w-full p-2.5 border border-gray-300 rounded-md" placeholder="Enter news title" required/>
              </div>
              ...
              <div className="mb-6">
                ...
                <textarea name="summary" value={formData.summary} onChange={handleTextAreaChange} rows={3} className="w-full p-2.5 border border-gray-300 rounded-md" placeholder="Enter a brief summary of the article"/>
              </div>
              ...
              <div className="mb-6">
                ...
                <textarea name="content" value={formData.content} onChange={handleTextAreaChange} rows={10} className="w-full p-2.5 border border-gray-300 rounded-md" placeholder="Enter the content of the article" required/>
              </div>
              ...
              <div className="mb-6">
                ...
                <textarea name="key_lessons" value={formData.key_lessons} onChange={handleTextAreaChange} rows={3} className="w-full p-2.5 border border-gray-300 rounded-md" placeholder="Enter key takeaways or lessons (separate with semicolons for multiple items)"/>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                {formData.status === 'published' && (<div>
                    ...
                    <div className="relative">
                      <input type="date" name="publish_date" value={formatDate(formData.publish_date || new Date())} onChange={handleDateChange} className="w-full p-2.5 border border-gray-300 rounded-md" required={formData.status === 'published'}/>
                    </div>
                  </div>)}<label className="block text-sm font-medium mb-1">
                    Category
                  </label>
                  <div className="relative">
                    <select name="category" value={formData.category} onChange={handleChange} className="w-full p-2.5 border border-gray-300 rounded-md appearance-none">
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-3/4">
                ...
                <div className="flex justify-end items-center mb-4">
                  <div className="flex items-center">
                    <button type="button" onClick={toggleMediaUrlMode} className={`text-sm py-1 px-3 rounded-md ${useMediaUrl
        ...
        <div className="border-2 border-dashed border-gray-300 p-6 rounded-md text-center mb-6">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <div className="w-full max-w-md mt-3">
                        <input type="url" value={newMedia.url} onChange={handleMediaUrlChange} className="w-full p-2.5 border border-gray-300 rounded-md" placeholder="https://example.com/image.jpg"/>
                      </div>
                      
                      {newMedia.url && (<div className="mt-4 p-4 bg-gray-50 rounded-md w-full max-w-md">
                          <div className="flex items-center mb-3">
                            {newMedia.type === 'image' ? (<ImageIcon className="w-5 h-5 text-blue-500 mr-2"/>) : (<FileVideo className="w-5 h-5 text-purple-500 mr-2"/>)}
                            <span className="text-sm font-medium">Preview</span>
                            <button type="button" onClick={removeNewMedia} className="ml-auto text-red-500 hover:text-red-700">
                              <X className="w-4 h-4"/>
                            </button>
                          </div>
                          
                          {/* URL preview - only for images */}
                          {newMedia.type === 'image' && (<div className="mb-4 border rounded overflow-hidden">
                              <img src={newMedia.url} alt="Preview" className="w-full h-auto max-h-60 object-contain" onLoad={() => console.log(`Successfully loaded image: ${newMedia.url}`)} onError={(e) => {
                        ...
                        handleImageError(e, fallbackText);
                          ...
                          <div className="grid grid-cols-1 gap-4">
                            ...
                            <div className="flex items-center justify-start">
                              <input type="checkbox" name="cover" id="mediaCoverUrl" checked={newMedia.cover} onChange={handleMediaChange} className="mr-2"/>
                            ...
                            <div className="flex justify-end">
                              <button type="button" onClick={addMedia} disabled={!newMedia.url || !newMedia.title} className="bg-green-700 hover:bg-green-800 text-white py-2 px-4 rounded-md text-sm flex items-center">
        ...
        // File upload option
        <div className="border-2 border-dashed border-gray-300 p-6 rounded-md text-center mb-6 cursor-pointer" onClick={!newMedia.file ? triggerFileInput : undefined}>
                    {!newMedia.file ? (<div className="flex flex-col items-center justify-center">
                      ...
                      </div>) : (<div className="p-4 bg-gray-50 rounded-md">
                    ...
                    removeNewMedia();
                        ...
                        {isUploading && (<div className="mb-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <input type="text" name="title" value={newMedia.title} onChange={handleMediaChange} placeholder="Media title" className="w-full p-2.5 border border-gray-300 rounded-md" onClick={(e) => e.stopPropagation()}/>
                          </div>
                          
                          <div className="flex items-center">
                            <input type="checkbox" name="cover" id="mediaCover" checked={newMedia.cover} onChange={handleMediaChange} className="mr-2" onClick={(e) => e.stopPropagation()}/>
                    ...
                    e.stopPropagation();
                    addMedia();
                }} disabled={isUploading || !newMedia.title} className="bg-green-700 hover:bg-green-800 text-white py-2 px-4 rounded-md text-sm flex items-center">
                              {isUploading ? (<>
                ...
                {formData.media.items.length === 0 ? (<p className="text-gray-500 text-sm italic mb-4">No media added yet. Upload images or videos to enhance your article.</p>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                    {formData.media.items.map(media => (<div key={media.id} className={`border rounded-md overflow-hidden ${selectedMedia && selectedMedia.id === media.id ? 'ring-2 ring-green-500' : ''}`}>
                        {/* Media preview */}
                        <div className="h-32 bg-gray-100 flex items-center justify-center overflow-hidden cursor-pointer relative" onClick={() => selectMedia(media.id)}>
                          {media.type === 'image' ? (<img src={media.url} alt={media.title} className="object-cover w-full h-full" onLoad={() => console.log(`Successfully loaded image: ${media.url}`)} onError={(e) => {
                        ...
                        handleImageError(e, fallbackText);
                              ...
                              {media.thumbnailUrl ? (<img src={media.thumbnailUrl} alt={media.title} className="object-cover w-full h-full" onLoad={() => console.log(`Successfully loaded video thumbnail: ${media.thumbnailUrl}`)} onError={(e) => {
                          ...
                          {media.cover && (<div className="absolute top-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded-md">
                              Cover
                            </div>)}
                        </div>
                        ...
                        <div className="p-3">
                          ...
                          <div className="flex flex-wrap gap-1 text-xs">
                            ...
                            {media.size > 0 && (<span className="px-2 py-1 bg-gray-100 rounded-full">
                        ...
                        {selectedMedia && selectedMedia.id === media.id && (<div className="p-3 border-t">
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="w-full md:w-3/4">
                <div className="mb-4 flex justify-between items-center">
                  ...
                  <button type="button" onClick={() => setShowTagModal(true)} className="text-sm flex items-center text-green-700 hover:text-green-800">
                ...
                <div className="bg-gray-50 p-4 rounded-md mb-4">
                  <div className="flex flex-wrap gap-2">
                    {tags.length === 0 ? (<p className="text-gray-500 text-sm">No tags available. Create your first tag with the button above.</p>) : (tags.map(tag => (<div key={tag.id} onClick={() => toggleTag(tag.id)} className={`px-3 py-2 rounded-full cursor-pointer flex items-center ${selectedTags.includes(tag.id)
                ? 'bg-green-100 text-green-800 border border-green-300'
                : 'bg-gray-100 text-gray-700 border border-gray-200 hover:bg-gray-200'}`}>
                          <TagIcon className="w-3 h-3 mr-1"/>
                          <span className="text-sm">{tag.name}</span>
                          {selectedTags.includes(tag.id) && (<Check className="w-3 h-3 ml-1 text-green-600"/>)}
                        </div>)))}
                  </div>
                </div>
              </div>
          ...
          <div className="flex justify-end space-x-3 p-6 border-t border-gray-200">
            <Link href="/news" className="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
              Cancel
            </Link>
        ...
        {/* Add Tag Modal */}
        {showTagModal && (<div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-96 max-w-full">
              ...
              <div className="mb-4">
                ...
                <input type="text" value={newTagName} onChange={(e) => setNewTagName(e.target.value)} className="w-full p-2.5 border border-gray-300 rounded-md" placeholder="Enter tag name"/>
              </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                setShowTagModal(false);
                setNewTagName('');
            }} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50" disabled={addingTag}>
                  Cancel
                </button>
                <button type="button" onClick={handleAddTag} className="px-4 py-2 bg-green-700 rounded-md text-white hover:bg-green-800 flex items-center" disabled={addingTag || !newTagName.trim()}>
                  {addingTag ? (<>
                      <Loader className="w-4 h-4 mr-2 animate-spin"/>
                      Adding...
                    </>) : (<>
