    ...
    return (<div className="p-6">
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                <div>
                  ...
                  <Input placeholder="Enter role name" value={newRole.name} onChange={(e) => setNewRole({ ...newRole, name: e.target.value })}/>
                </div>
                <div>
                  <label className="text-sm font-medium mb-1 block">
                    Description *
                  </label>
                  <Input placeholder="Enter role description" value={newRole.description} onChange={(e) => setNewRole({ ...newRole, description: e.target.value })}/>
                </div>
              </div>
              <DialogFooter>
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <Input className="pl-10" placeholder="Search roles..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
          </div>
        </div>

        {/* Loading Skeletons */}
        {isLoading && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map((index) => (<Card key={index} className="p-6">
        ...
        {/* Roles Grid */}
        {!isLoading && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredRoles.length > 0 ? (filteredRoles.map((role) => (<Card key={role.id} className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                          ...
                          <div className="flex gap-2">
                            <Button variant="outline" size="sm" className="text-primary-green hover:bg-green-50" title="Edit role" onClick={() => handleEditClick(role)}>
                              <Edit2 className="h-4 w-4"/>
                            </Button>
                            <Button variant="outline" size="sm" className="text-red-600 hover:bg-red-50" title="Delete role" onClick={() => handleDeleteClick(role)}>
                      ...
                      </Card>))) : (<div className="col-span-full text-center py-10">
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div>
                ...
                <Input placeholder="Enter role name" value={editRole.name} onChange={(e) => setEditRole({ ...editRole, name: e.target.value })}/>
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">
                  Description *
                </label>
                <Input placeholder="Enter role description" value={editRole.description} onChange={(e) => setEditRole({ ...editRole, description: e.target.value })}/>
              </div>
            </div>
            <DialogFooter>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                {selectedRole?.usersCount && selectedRole.usersCount > 0 && (<p className="mt-2 text-red-600">
