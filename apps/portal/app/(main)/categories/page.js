...
export default function ProjectCategoriesPage() {
    ...
    const fetchCategories = async () => {
        ...
        try {
            ...
            const categoriesWithColors = response.data.categories.map((category) => ({
                ...category,
    ...
    return (<div className="p-6">
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input id="name" placeholder="Enter category name" value={newCategory.name} onChange={(e) => setNewCategory({ ...newCategory, name: e.target.value })}/>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="color">Color (for display only)</Label>
                  <div className="flex flex-wrap gap-2">
                    {colorOptions.map((color) => (<div key={color.value} className={`w-8 h-8 rounded-full cursor-pointer ${color.value.split(' ')[0]} border-2 ${newCategory.color === color.value ? 'border-black' : 'border-transparent'}`} onClick={() => setNewCategory({ ...newCategory, color: color.value })} title={color.name}/>))}
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="description">Description</Label>
                  <Textarea id="description" placeholder="Enter category description" value={newCategory.description} onChange={(e) => setNewCategory({ ...newCategory, description: e.target.value })}/>
                </div>
              </div>
              <DialogFooter>
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <Input className="pl-10" placeholder="Search categories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
          </div>
        </div>

        {/* Loading State */}
        {isLoading && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1, 2, 3, 4, 5, 6].map((index) => (<Card key={index} className="relative overflow-hidden">
        ...
        {/* Categories Grid */}
        {!isLoading && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredCategories.map((category) => (<Card key={category.id} className="relative overflow-hidden">
                  ...
                  <span className="text-sm text-gray-500">
                    Created: {formatDate(category.created_at)}
                        ...
                        <div className="flex gap-2">
                          <Button variant="ghost" size="sm" className="text-gray-500 hover:text-gray-700" onClick={() => handleEditClick(category)} title="Edit category">
                            <Edit2 className="w-4 h-4"/>
                          </Button>
                          <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-700" onClick={() => handleDeleteClick(category)} title="Delete category">
        ...
        {!isLoading && filteredCategories.length === 0 && (<div className="text-center py-12">
              <AlertCircle className="w-12 h-12 text-gray-400 mx-auto mb-4"/>
              <h3 className="text-lg font-medium text-gray-900">No categories found</h3>
              {searchQuery ? (<p className="text-gray-500">No categories match your search criteria.</p>) : (<p className="text-gray-500">Start by adding your first project category.</p>)}
            </div>)}
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input id="edit-name" placeholder="Enter category name" value={editCategory.name} onChange={(e) => setEditCategory({ ...editCategory, name: e.target.value })}/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-color">Color (for display only)</Label>
                <div className="flex flex-wrap gap-2">
                  {colorOptions.map((color) => (<div key={color.value} className={`w-8 h-8 rounded-full cursor-pointer ${color.value.split(' ')[0]} border-2 ${editCategory.color === color.value ? 'border-black' : 'border-transparent'}`} onClick={() => setEditCategory({ ...editCategory, color: color.value })} title={color.name}/>))}
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="edit-description">Description</Label>
                <Textarea id="edit-description" placeholder="Enter category description" value={editCategory.description} onChange={(e) => setEditCategory({ ...editCategory, description: e.target.value })}/>
              </div>
            </div>
            <DialogFooter>
