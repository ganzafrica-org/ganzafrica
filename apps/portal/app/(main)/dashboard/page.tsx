...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then((response) => {
        ...
        const chartData = relevantMonths.map(
          (month) =>
        ...
        const pendingTotal = projectsArray.filter(
          (p) => p && (p.status === "planned" || p.status === "pending"),
        ).length;
        const activeTotal = projectsArray.filter(
          (p) => p && (p.status === "active" || p.status === "in-progress"),
        ).length;
        ...
        if (chartData && chartData.length >= 2) {
          ...
          const completedTrend =
            firstCompleted > 0 ? ((lastCompleted - firstCompleted) / firstCompleted) * 100 : 0;

          const pendingTrend =
            firstPending > 0 ? ((lastPending - firstPending) / firstPending) * 100 : 0;

          const activeTrend =
            firstActive > 0 ? ((lastActive - firstActive) / firstActive) * 100 : 0;
    ...
    apiClient
      ...
      .then((response) => {
        ...
        const activities = sortedProjects.slice(0, 4).map((project) => {
          ...
          return {
            user: {
              ...
              avatar:
                project.createdBy?.avatar || project.creator?.avatar || "/avatars/default.jpg",
            },
            action: isUpdated
  ...
  const maxProjectValue =
    projectStatsData.length > 0
      ? Math.max(
          ...projectStatsData.map((item) =>
            Math.max(item.completed || 0, item.pending || 0, item.total || 0),
          ),
        )
      : 10;
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="mb-6 md:mb-8">
        <h1 className="text-xl md:text-2xl font-semibold mb-2 dark:text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
        <Card className="relative shadow-sm overflow-hidden dark:bg-gray-800">
          ...
          <CardHeader className="pb-0 pt-4 px-4">
            ...
            <CardTitle className="text-xl md:text-2xl font-semibold mt-1 md:mt-2 dark:text-white">
              {totalUsers}
            </CardTitle>
          </CardHeader>
          <CardFooter className="flex items-center pt-0 pb-4 px-4">
            <div className="flex items-center">
              ...
              <span className="text-xs md:text-sm text-black dark:text-white ml-1">
                since last week
              </span>
            </div>
          </CardFooter>
        ...
        <Card className="relative shadow-sm overflow-hidden dark:bg-gray-800">
          ...
          <CardFooter className="flex items-center pt-0 pb-4 px-4">
            <div className="flex items-center">
              <span
                className={`text-xs md:text-sm ${trendData.completedTrend >= 0 ? "text-primary-green" : "text-red-500"} font-medium`}
              >
                {trendData.completedTrend >= 0
                  ? `↑ ${trendData.completedTrend}%`
                  : `↓ ${Math.abs(trendData.completedTrend)}%`}
              </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
        ...
        <Card className="shadow-sm dark:bg-gray-800">
          <CardHeader className="flex flex-row items-center justify-between pb-2 pt-4 px-4">
            <div>
              <CardTitle className="text-base md:text-lg font-semibold dark:text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
          ...
          {recentActivities.length > 0 ? (
            recentActivities.map((activity, index) => (
              <div key={index} className="flex items-start px-4 py-3 md:py-4">
                ...
                <div
                  className={`pb-3 md:pb-4 ${index < recentActivities.length - 1 ? "border-b dark:border-gray-700 w-full" : "w-full"}`}
                >
        ...
        <div className="lg:col-span-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
          <div className="border-b border-gray-200 dark:border-gray-700 px-4 py-3 md:py-4 flex justify-between items-center">
            ...
            <a
              href="/projects"
              className="text-xs md:text-sm text-emerald-600 dark:text-emerald-400 hover:underline"
            >
              View all
            </a>
          </div>
          ...
          {activeProjects.length > 0 ? (
            activeProjects.map((project, index) => (
              <div key={index} className="flex items-start px-4 py-3 md:py-4">
                ...
                <div
                  className={`pb-3 md:pb-4 ${index < activeProjects.length - 1 ? "border-b dark:border-gray-700 w-full" : "w-full"}`}
                >
                  <h4 className="text-sm md:text-base font-medium dark:text-white line-clamp-1">
                    {project.name}
                  </h4>
