...
import { Card, CardHeader, CardTitle, CardDescription, CardFooter, CardContent } from '@workspace/ui/components/card';
...
export default function DashboardPage() {
            ...
            userData.forEach(user => {
                ...
                if (user.createdAt || user.created_at) {
                    ...
                    if (daysDiff <= 7)
                        ...
                        return; // Skip users created more than 28 days ago
    ...
    const maxProjectValue = projectStatsData.length > 0
        ? Math.max(...projectStatsData.map(item => Math.max(item.completed || 0, item.pending || 0, item.total || 0)))
        : 10;
    const yAxisDomain = [0, Math.max(10, Math.ceil(maxProjectValue * 1.2))];
    return (<div className="p-4 sm:p-6 md:p-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
      {/* Welcome Section */}
          ...
          <CardContent className="px-2 md:px-4">
            <ChartContainer config={projectChartConfig}>
              <ResponsiveContainer width="100%" height={250}>
                <LineChart accessibilityLayer data={projectStatsData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                  <CartesianGrid vertical={false}/>
                  <XAxis dataKey="month" tickLine={false} tickMargin={10} axisLine={false} tick={{ fontSize: 11 }}/>
                  <YAxis tickLine={false} axisLine={false} domain={yAxisDomain} tick={{ fontSize: 11 }} label={{ value: 'Number of Projects', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fontSize: 12 }, dx: -10 }}/>
                  <ChartTooltip cursor={{ strokeDasharray: '3 3' }} content={<ChartTooltipContent indicator="dashed"/>}/>
                  <Line type="monotone" dataKey="total" stroke="var(--color-total)" strokeWidth={1} strokeDasharray="3 3" dot={{ fill: "var(--color-total)", r: 3 }} activeDot={{ r: 5 }}/>
                  <Line type="monotone" dataKey="completed" stroke="var(--color-completed)" strokeWidth={2} dot={{ fill: "var(--color-completed)", r: 4 }} activeDot={{ r: 6 }}/>
                  <Line type="monotone" dataKey="pending" stroke="var(--color-pending)" strokeWidth={2} dot={{ fill: "var(--color-pending)", r: 4 }} activeDot={{ r: 6 }}/>
                  <Line type="monotone" dataKey="active" stroke="var(--color-active)" strokeWidth={2} dot={{ fill: "var(--color-active)", r: 4 }} activeDot={{ r: 6 }}/>
                  
                  {/* Add a reference line to highlight the trend */}
                  {projectStatsData.length >= 2 && (<ReferenceLine stroke="#009758" strokeDasharray="3 3" segment={[
                {
        ...
        <Card className="shadow-sm dark:bg-gray-800">
          ...
          <CardContent className="px-2 md:px-4">
            <ChartContainer config={engagementChartConfig}>
              <ResponsiveContainer width="100%" height={250}>
                <LineChart accessibilityLayer data={userEngagementData} margin={{
            top: 5,
            left: 0,
            right: 10,
            bottom: 5
                  ...
                  <CartesianGrid vertical={false}/>
                  <XAxis dataKey="week" tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 11 }}/>
                  <YAxis tickLine={false} axisLine={false} domain={[0, 'dataMax + 5']} tick={{ fontSize: 11 }} label={{ value: 'Number of Users', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fontSize: 12 }, dx: -10 }}/>
                  <ChartTooltip cursor={{ strokeDasharray: '3 3' }} content={<ChartTooltipContent />}/>
                  <Line dataKey="total" type="monotone" stroke="var(--color-total)" strokeWidth={1} strokeDasharray="3 3" dot={{
            fill: "var(--color-total)",
            r: 3
        }} activeDot={{
            r: 5,
        }}/>
                  <Line dataKey="active" type="monotone" stroke="var(--color-active)" strokeWidth={2} dot={{
            fill: "var(--color-active)",
            r: 4
        }} activeDot={{
            r: 6,
        }}/>
                  <Line dataKey="inactive" type="monotone" stroke="var(--color-inactive)" strokeWidth={2} dot={{
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
          ...
          {recentActivities.length > 0 ? (recentActivities.map((activity, index) => (<div key={index} className="flex items-start px-4 py-3 md:py-4">
                ...
                <div className={`pb-3 md:pb-4 ${index < recentActivities.length - 1 ? "border-b dark:border-gray-700 w-full" : "w-full"}`}>
                  ...
                  <div className="flex items-center mt-1">
                    {activity.timeAgo.isRecent ? (<div className="flex items-center">
                        <div className="w-2 h-2 bg-green-500 rounded-full mr-2"></div>
                        <span className="text-xs text-gray-500 dark:text-gray-400">{activity.timeAgo.text}</span>
                      </div>) : (<span className="text-xs text-gray-500 dark:text-gray-400">{activity.timeAgo.text}</span>)}
                  </div>
                </div>
              </div>))) : (<div className="px-4 py-6 md:py-8 text-center">
        ...
        <div className="lg:col-span-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
          ...
          {activeProjects.length > 0 ? (activeProjects.map((project, index) => (<div key={index} className="flex items-start px-4 py-3 md:py-4">
                <Avatar className="mr-2 md:mr-3 h-6 w-6 md:h-8 md:w-8 flex-shrink-0">
