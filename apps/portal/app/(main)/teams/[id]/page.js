...
import { ArrowLeft, Mail, Globe, Tag, Clock, User, Briefcase, AlertCircle, ExternalLink } from 'lucide-react';
    ...
    return (<div className="p-6 max-w-full">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <span className={`px-2 py-1 text-xs font-medium rounded-full ${team.team_type_id === 1 ? 'bg-blue-100 text-blue-800' :
        ...
        <Link href={`/teams/edit-team/${team.id}`} className="px-4 py-2 bg-green-700 rounded text-sm font-medium text-white hover:bg-green-800">
          Edit Team Member
        </Link>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-1/4 bg-white p-4 rounded border border-gray-200">
          <ul>
            <li className="mb-6">
              <button onClick={() => setActiveTab('profile')} className={`w-full text-left flex items-start ${activeTab === 'profile' ? 'text-green-700' : 'text-gray-700'}`}>
            ...
            <li className="mb-6">
              <button onClick={() => setActiveTab('skills')} className={`w-full text-left flex items-start ${activeTab === 'skills' ? 'text-green-700' : 'text-gray-700'}`}>
                <div className="flex-shrink-0 mt-1">
            ...
            <li className="mb-6">
              <button onClick={() => setActiveTab('bio')} className={`w-full text-left flex items-start ${activeTab === 'bio' ? 'text-green-700' : 'text-gray-700'}`}>
                <div className="flex-shrink-0 mt-1">
        ...
        <div className="w-full md:w-3/4">
          {/* Profile tab */}
          {activeTab === 'profile' && (<div className="bg-white p-6 rounded-lg border border-gray-200">
              {/* Profile image and basic info */}
              <div className="flex flex-col md:flex-row gap-6 mb-8">
                {/* Profile photo */}
                <div className="w-full md:w-1/3">
                  <div className="aspect-square rounded-lg overflow-hidden mb-4">
                    <img src={team.photo_url || "/api/placeholder/400/400"} alt={team.name} className="w-full h-full object-cover"/>
                  </div>
                </div>
                ...
                <div className="w-full md:w-2/3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {team.profile_link && (<div className="p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center mb-2">
                          <Globe className="w-4 h-4 text-gray-500 mr-2"/>
                          <span className="text-sm text-gray-500">Profile Link</span>
                        </div>
                        <a href={team.profile_link} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-medium flex items-center">
          ...
          {/* Skills tab */}
          {activeTab === 'skills' && (<div className="bg-white p-6 rounded-lg border border-gray-200">
              <h2 className="text-xl font-bold mb-6">Skills & Expertise</h2>
              
              {team.skills && team.skills.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {team.skills.map((skill, index) => (<div key={index} className="p-4 bg-gray-50 rounded-lg border-l-4 border-green-700">
          ...
          {/* Bio tab */}
          {activeTab === 'bio' && (<div className="bg-white p-6 rounded-lg border border-gray-200">
              <h2 className="text-xl font-bold mb-6">Biography</h2>
              
              {team.bio ? (<div className="p-4 bg-gray-50 rounded-lg">
