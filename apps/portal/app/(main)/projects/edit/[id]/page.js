...
const EditProjectPage = () => {
        ...
        const fetchProjectData = async () => {
            try {
                ...
                const processedFormData = {
                    ...
                    members: Array.isArray(projectData.members) ? projectData.members.map(member => ({
                        team_id: member.team_id,
                        role: member.role
                    })) : [],
                    partners: Array.isArray(projectData.partners) ? projectData.partners.map(partner => ({
    ...
    const MediaPreview = ({ media }) => {
        if (media.type === 'image') {
          ...
          {media.thumbnailUrl ? (<img src={media.thumbnailUrl} alt={media.title} className="object-cover w-full h-full"/>) : (<div className="bg-gray-200 w-full h-full flex items-center justify-center">
    ...
    const VideoPreviewModal = () => {
        ...
        <div className="relative w-full max-w-4xl mx-auto p-4">
          ...
          <div className="bg-black rounded-lg overflow-hidden">
            ...
            videoPreviewUrl.includes('youtube.com') || videoPreviewUrl.includes('youtu.be') ? (<iframe src={videoPreviewUrl.replace('watch?v=', 'embed/')} className="w-full h-[80vh]" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>) : (<video src={videoPreviewUrl} controls autoPlay className="w-full h-auto max-h-[80vh]"/>)) : (
            // For blob URLs, show the video directly
            <video src={videoPreviewUrl} controls autoPlay className="w-full h-auto max-h-[80vh]"/>)}
          </div>
        </div>
      </div>);
    };
    ...
    if (initialLoading) {
        return (<div className="max-w-7xl mx-auto p-6 flex items-center justify-center min-h-[80vh]">
    ...
    return (<div className="max-w-7xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md">
        ...
        <div className="flex justify-end space-x-3 p-6 border-t border-gray-200">
          <button type="button" className="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50" onClick={() => router.push(`/projects/${projectId}`)}>
            Cancel
          </button>
