  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const projectData = {
        ...
        impacted_people: formData.impacted_people
          ? Number(formData.impacted_people)
          : undefined,
                  ...
                  <div className="border border-gray-300 rounded-md p-3 min-h-32">
                    {teamMembers
                      ...
                      .map((member) => (
                        <li
                          key={member.id}
                          className="flex justify-between items-center text-sm mb-2 list-none"
                        >
                          <span>
                            <span className="font-medium">{member.name}</span> -{" "}
                            {member.role}
