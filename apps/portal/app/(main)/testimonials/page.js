...
import { Grid2X2, List, Trash2, Edit2, Eye, Quote, Search, Filter, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ArrowRight } from 'lucide-react';
...
export default function TestimonialsPage() {
    ...
    const [activeCategory, setActiveCategory] = useState('All Testimonials');
    ...
    const filteredTestimonials = testimonials.filter(testimonial => {
        ...
        // Search filter
        if (searchQuery) {
            return (testimonial.author_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const renderMediaPreview = (imageUrl, authorName) => {
        if (!imageUrl || imageUrl === 'string' || imageErrors[imageUrl]) {
            return null;
        }
        // For localhost URLs, use a regular img tag
        if (imageUrl.includes('localhost')) {
            return (<div className="grid grid-cols-1 gap-2 mt-4">
          <div className="relative aspect-video rounded-lg overflow-hidden">
            <img src={imageUrl} alt={`Photo of ${authorName}`} className="object-cover w-full h-full" onError={() => handleImageError(imageUrl)}/>
          </div>
        </div>);
        }
    ...
    const renderAvatar = (testimonial) => {
        ...
        return (<Avatar className="h-16 w-16 ring-2 ring-offset-2 ring-primary-green">
        {hasValidImage ? (<>
            <AvatarImage src={testimonial.image} alt={testimonial.author_name} onError={() => handleImageError(testimonial.image)}/>
    ...
    const renderGridView = () => (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        Array(6).fill(0).map((_, index) => (<Card key={index} className="p-6 flex flex-col relative overflow-hidden">
                ...
                </Card>))) : (filteredTestimonials.map((testimonial) => (<Card key={testimonial.id} className="p-6 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-end mt-4 border-t pt-4">
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" className="text-gray-600 hover:text-gray-900 hover:bg-gray-100" title="View testimonial" onClick={() => handleView(testimonial)}>
                        <Eye className="h-4 w-4 mr-2"/>
                        View
                      </Button>
                      <Button variant="outline" size="sm" className="text-primary-green hover:bg-green-50" title="Edit testimonial" onClick={() => handleEdit(testimonial)}>
                        <Edit2 className="h-4 w-4 mr-2"/>
                        Edit
                      </Button>
                      <Button variant="outline" size="sm" className="text-red-600 hover:bg-red-50" title="Delete testimonial" onClick={() => handleDeleteClick(testimonial)}>
    ...
    const renderListView = () => (<div className="space-y-4">
        {isLoading ? (
        // Skeleton loading state
        Array(4).fill(0).map((_, index) => (<Card key={index} className="p-6 relative">
                ...
                </Card>))) : (filteredTestimonials.map((testimonial) => (<Card key={testimonial.id} className="p-6 relative">
                  <div className="absolute top-4 right-4 text-gray-400">
                    <Quote size={24}/>
                  </div>
                  <div className="flex items-start gap-6">
    ...
    return (<div className="p-6">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {categories.map((category) => (<Button key={category} variant={activeCategory === category ? 'secondary' : 'outline'} onClick={() => setActiveCategory(category)} className="whitespace-nowrap">
                {category}
              </Button>))}
        </div>
        ...
        <div className="flex justify-end mb-4">
          <div className="relative w-64">
            ...
            <input type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded block w-full pl-10 p-2.5" placeholder="Search testimonials" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
          </div>
        ...
        <div className="flex justify-end mb-4">
          <div className="flex border rounded-lg">
            <Button variant={viewMode === 'grid' ? 'secondary' : 'ghost'} className="rounded-r-none" onClick={() => setViewMode('grid')} title="Grid view">
              <Grid2X2 className="h-4 w-4 mr-2"/>
              Grid
            </Button>
            <Button variant={viewMode === 'list' ? 'secondary' : 'ghost'} className="rounded-l-none" onClick={() => setViewMode('list')} title="List view">
        ...
        {!isLoading && filteredTestimonials.length === 0 && (<div className="text-center py-10 bg-white rounded-lg">
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={handleDeleteConfirm} className="bg-red hover:bg-red-700 text-white">
                Delete
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
