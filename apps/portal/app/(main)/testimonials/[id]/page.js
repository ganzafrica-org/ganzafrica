...
export default function TestimonialViewPage({ params }) {
    ...
    const renderRatingStars = (rating) => {
        return (<div className="flex items-center">
          {Array.from({ length: 5 }).map((_, index) => (<Star key={index} className={`h-5 w-5 ${index < rating ? 'text-primary-orange fill-yellow-400' : 'text-gray-300'}`}/>))}
    ...
    if (!testimonial) {
        return (<div className="p-6 text-center">
          ...
          <Button variant="link" onClick={() => router.push('/testimonials')} className="mt-4">
    ...
    return (<div className="p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => router.push(`/testimonials/edit/${testimonial.id}`)} className="bg-primary-green hover:bg-green-700">
