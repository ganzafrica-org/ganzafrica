...
export default function TestimonialsPage() {
  ...
  const categories = ["All Testimonials", "Fellow", "CEO", "Manager", "5 Star", "4 Star", "3 Star"];
  ...
  const filteredTestimonials = testimonials.filter((testimonial) => {
    ...
    if (activeCategory === "All Testimonials") {
      ...
      activeCategory === "5 Star" ||
      activeCategory === "4 Star" ||
      activeCategory === "3 Star"
    ) {
      const rating = parseInt((activeCategory ?? "").split(" ")[0]);
      if (testimonial.rating !== rating) return false;
    } else if (
      !testimonial.position.toLowerCase().includes(activeCategory.toLowerCase()) &&
  ...
  const renderAvatar = (testimonial: Testimonial) => {
    const hasValidImage =
      testimonial.image && testimonial.image !== "string" && !imageErrors[testimonial.image];
      ...
      <Avatar className="h-16 w-16 ring-2 ring-offset-2 ring-primary-green">
        {hasValidImage ? (
          <>
            ...
            />
            <AvatarFallback className="bg-primary-green text-white">{initials}</AvatarFallback>
          </>
        ) : (
          <AvatarFallback className="bg-primary-green text-white">{initials}</AvatarFallback>
        )}
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-3">
          <Link
            href="/testimonials/add-testimonial"
