...
export default function AddMediaPage() {
    ...
    return (<div className="p-8">
      ...
      <div className="max-w-3xl mx-auto">
        <div className={`border-2 border-dashed rounded-lg p-8 text-center ${isDragging
            ? 'border-green-500 bg-green-50'
            : 'border-gray-300 hover:border-gray-400'}`} onDragEnter={handleDragEnter} onDragOver={handleDragEnter} onDragLeave={handleDragLeave} onDrop={handleDrop}>
          ...
          <input type="file" multiple onChange={handleFileSelect} className="hidden" id="file-upload" accept={ALLOWED_FILE_TYPES.join(',')}/>
        ...
        {/* Selected Files */}
        {files.length > 0 && (<div className="mt-8">
            <h3 className="text-lg font-medium mb-4">Selected Files</h3>
            <div className="space-y-3">
              {files.map((file, index) => (<div key={index} className={`flex items-center justify-between p-3 bg-white rounded-lg border ${file.error ? 'border-red-300 bg-red-50' : ''}`}>
                  <div className="flex items-center gap-3 flex-1">
                    <span className="text-2xl">{getFileTypeIcon(file)}</span>
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <p className="font-medium">{file.name}</p>
                        {file.error && (<div className="flex items-center text-red-600 text-sm">
                      ...
                      {uploading && !file.error && (<Progress value={uploadProgress[file.name] || 0} className="mt-2"/>)}
                    </div>
                  </div>
                  <Button variant="ghost" size="icon" onClick={() => removeFile(index)} title="Remove file" disabled={uploading}>
                    <X className="w-4 h-4"/>
                  </Button>
                </div>))}
            </div>
        ...
        <div className="mt-8 flex justify-end gap-3">
          ...
          <Button onClick={handleUpload} disabled={files.length === 0 || uploading || files.every(f => f.error)} className="bg-green-700 hover:bg-green-800">
