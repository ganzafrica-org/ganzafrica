...
export function SignupForm({ userType }: SignupFormProps) {
  ...
  const handleSignup = async (data: {
    ...
    try {
      ...
      // Include role_type for alumni registration
      ...
      // If registering as alumni, pass the role type
      if (userType === "alumni") {
        requestData.role_type = "alumni";
      }
      ...
      if (
        response.status === 200 ||
        response.status === 201 ||
        (response.data && (response.data.user || response.data.message))
      ) {
        // Show success toast
        showSuccessToast({
          title: "Success",
        ...
        // If alumni signup, redirect to alumni login
        setTimeout(() => {
          setIsLoading(false);
          if (userType === "alumni") {
            router.push("/login?user=alumni");
          } else {
            router.push("/login");
          }
        }, 500);
      ...
      if (
        ...
        showErrorToast({
          ...
          message:
            "The registration request took too long. Your account may have been created. Please try logging in. If login fails, try registering again.",
        });
        setIsLoading(false);
