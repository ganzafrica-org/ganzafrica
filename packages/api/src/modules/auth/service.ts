        ...
        if (user.account_locked) {
            ...
            if (user.last_failed_attempt) {
                ...
                if (new Date() > lockoutExpiry) {
                    ...
                    logger.warn('Authentication failed: Account locked', { userId: user.id });
                    throw new Error('Account is temporarily locked due to multiple failed attempts');
                }
            } else {
                logger.warn('Authentication failed: Account locked', { userId: user.id });
                throw new Error('Account is locked. Please reset your password or contact support.');
            }
        }
...
export async function validateSession(token: string): Promise<any> {
    try {
        ...
        if (!session || !session.is_valid) {
            logger.warn('Session validation failed: Invalid session', { sessionId: payload.sessionId });
            return null;
        }

        // Check if session is expired
        if (session.expires_at < new Date()) {
            logger.warn('Session validation failed: Expired session', { sessionId: payload.sessionId });
            return null;
        }
        ...
        if (!user || !user.is_active) {
            logger.warn('Session validation failed: User inactive', { userId: session.user_id });
            return null;
        }
 ...
 * @param ipAddress Client IP address
 */
export async function requestPasswordReset(input: RequestPasswordResetInput, ipAddress: string): Promise<boolean> {
    // Find the user by email
    const user = await db.query.users.findFirst({
        where: eq(users.email, input.email.toLowerCase()),
    });
    ...
    if (!user) {
        ...
        logger.info('Password reset requested for non-existent user', { email: input.email });
        return true;
    }
...
export async function resetPassword(input: ResetPasswordInput): Promise<boolean> {
    return withDbTransaction(async (txDb) => {
        ...
        if (resetToken.used) {
            logger.warn('Password reset failed: Token already used', { userId: resetToken.user_id });
            return false;
        }

        if (resetToken.expires_at < new Date()) {
            logger.warn('Password reset failed: Token expired', { userId: resetToken.user_id });
            return false;
        }
...
export async function verifyEmail(token: string): Promise<boolean> {
    return withDbTransaction(async (txDb) => {
        ...
        logger.info('Email verification successful', { userId: verificationToken.user_id });
