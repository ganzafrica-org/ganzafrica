  ...
  try {
    ...
    if (targetUser.role_id === adminRoleId) {
      ...
      try {
        ...
        await db.execute(sql`CREATE SEQUENCE IF NOT EXISTS audit_logs_id_seq`);
        ...
        console.log(
          `Note: Could not setup audit_logs (${seqError.message}). Will proceed with trigger disabled.`,
        );
      }
      ...
      try {
        ...
        console.log(
          `Warning: Could not disable trigger (${updateError.message}). Trying direct update...`,
        );
        try {
          await db.execute(
            sql`UPDATE users SET role_id = ${adminRoleId}, updated_at = NOW() WHERE id = ${targetUser.id}`,
          ...
          console.log(
            "Note: User already has admin role in user_roles table, so they should have admin access.",
          );
        }
      }
    }
