...
export async function createTeam(teamData: CreateTeamInput): Promise<TeamOutput> {
  try {
    ...
    const teamType = await db.select()
      .from(team_types)
    ...
    const createdTeam = await db.select()
      .from(teams)
  ...
  try {
    ...
    await db.update(teams)
      .set({
        ...
        photo_url: teamData.photo_url !== undefined ? teamData.photo_url : existingTeam[0].photo_url,
        ...
        profile_link: teamData.profile_link !== undefined ? teamData.profile_link : existingTeam[0].profile_link,
