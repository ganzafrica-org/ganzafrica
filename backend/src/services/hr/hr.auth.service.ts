...
export async function login(input: LoginInput): Promise<{ user: { id: string; email: string; role: HrRole }; tokens: AuthTokens }> {
  ...
  const accessToken = await signToken(
    { id: String(user.id), email: user.email, role: user.role as HrRole, type: "access" },
    env.ACCESS_TOKEN_EXPIRY,
  );
  const refreshToken = await signRefreshToken(
    { id: String(user.id), email: user.email, role: user.role as HrRole },
  ...
  return {
    user: { id: String(user.id), email: user.email, role: user.role as HrRole },
