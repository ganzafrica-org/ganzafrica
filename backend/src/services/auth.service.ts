 ...
 */
export async function createSession(userId: bigint, ipAddress: string, userAgent: string) {
    // Generate tokens
 ...
 */
export async function invalidateSession(tokenOrSessionId: string, isToken: boolean = true) {
    try {
        if (isToken) {
 ...
 */
export async function sendPasswordReset(userId: bigint, email: string, ipAddress: string) {
    try {
 ...
 */
export async function resetPassword(token: string, userId: bigint, newPassword: string) {
    return await withDbTransaction(async (txDb) => {
        // Verify token
