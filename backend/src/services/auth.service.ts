...
import { users, sessions, verification_tokens, password_reset_tokens } from '@/db/schema';
...
const JWT_REFRESH_SECRET = env.JWT_REFRESH_SECRET || 'your-default-jwt-refresh-secret-key-should-be-updated';
 ...
 */
export async function verifyToken(token: string, isRefresh: boolean = false): Promise<TokenPayload> {
    try {
        const secret = isRefresh ? JWT_REFRESH_SECRET : JWT_SECRET;
 ...
 */
export async function invalidateSession(tokenOrSessionId: string, isToken: boolean = true): Promise<boolean> {
    try {
        if (isToken) {
            try {
 ...
 * @returns {Promise<boolean>} - Result of operation
 */
export async function sendPasswordReset(userId: number, email: string, ipAddress: string): Promise<boolean> {
    try {
        const token = crypto.randomBytes(32).toString('hex');
        const tokenHash = await bcrypt.hash(token, SALT_ROUNDS);
...
export async function verifyEmailToken(token: string, userId: number): Promise<boolean> {
    return await withDbTransaction(async (txDb) => {
        const tokens = await txDb.select().from(verification_tokens)
            .where(
                and(
                    eq(verification_tokens.user_id, userId),
                    eq(verification_tokens.type, 'email'),
 ...
 */
export async function verifyPasswordResetToken(token: string, userId: number): Promise<any> {
 ...
 * @returns {Promise<boolean>} - Result of operation
 */
export async function resetPassword(token: string, userId: number, newPassword: string): Promise<boolean> {
    return await withDbTransaction(async (txDb) => {
        // Verify the token first
        let validToken = null;
        const tokens = await txDb.select().from(password_reset_tokens)
            .where(
                and(
                    eq(password_reset_tokens.user_id, userId),
                    eq(password_reset_tokens.used, false)
