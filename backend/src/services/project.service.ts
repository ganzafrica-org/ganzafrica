...
// Update project
export async function updateProject(id: number, projectData: UpdateProjectInput): Promise<ProjectOutput> {
    try {
        ...
        await db.update(projects)
            .set({
                ...projectData,
                status: projectData.status as "planned" | "active" | "completed" | "cancelled" | "on_hold" | undefined,
                updated_at: new Date()
            })
            .where(eq(projects.id, id));
...
export async function addProjectMember(projectId: number, memberData: ProjectMemberInput): Promise<ProjectMemberOutput> {
    try {
        // Check if project exists
        const existingProject = await db.select().from(projects)
            .where(eq(projects.id, projectId))
...
export async function listProjects(params: ProjectSearchParams): Promise<{ projects: ProjectOutput[], total: number }> {
    try {
        const { page = 1, limit = 10, search, sort_by = 'created_at', sort_order = 'desc', status, created_by, member_id, category_id } = params;
        ...
        if (status) {
            if (["planned", "active", "completed", "cancelled", "on_hold"].includes(status)) {
                whereConditions.push(eq(projects.status, status as "planned" | "active" | "completed" | "cancelled" | "on_hold"));
            }
        }
        ...
        if (result.length > 0) {
            ...
            for (const member of allMembers) {
                ...
                projectMemberMap.get(member.project_id)!.push(mapToProjectMemberOutput(member));
            }
        }
...
function validateRole(role: string): string {
    ...
    if (!allowedRoles.includes(role)) {
        throw new AppError(`Invalid role: ${role}. Allowed roles are: ${allowedRoles.join(', ')}`, 400);
    }
    return role;
}