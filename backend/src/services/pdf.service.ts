...
function drawHeader(doc: PDFKit.PDFDocument, docTitle: string) {
  ...
  for (const p of candidates) {
    try {
      if (fs.existsSync(p)) { doc.image(p, L, 40, { width: 80 }); logoDrawn = true; break; }
    } catch { /* skip */ }
  }
  if (!logoDrawn) {
  ...
  doc.font("Helvetica").fontSize(8).fillColor("#555555")
    .text("Reg No: N\u00B0 64/RGB/FDN/LP/07/2024", L, 59, { align: "right", width: W, lineBreak: false });
  doc.fontSize(8)
...
function drawEmployeeSection(doc: PDFKit.PDFDocument, data: PayslipData) {
  ...
  if (type === "rwf" || type === "rwf_usd") {
    if (data.staff_fellow_number) idRows.push({ label: "Employee No.:", value: data.staff_fellow_number });
    if (data.employee_id)         idRows.push({ label: "Employee ID:", value: data.employee_id });
  } else if (type === "wop_usd") {
    if (data.staff_fellow_number) idRows.push({ label: "Consultant ID:", value: data.staff_fellow_number });
...
function cell(
  ...
  y: number,
  w: number,
  opts: { bold?: boolean; color?: string; align?: "left" | "right"; size?: number; indent?: number } = {},
) {
...
function drawEarningsDeductionsTable(doc: PDFKit.PDFDocument, data: PayslipData): number {
  ...
  if (type === "rwf" || type === "rwf_usd") {
    ...
    if (data.gross_rwf) earnRows.push({ label: "Gross (RWF equivalent)", value: fmt(data.gross_rwf, "RWF") });
  } else if (type === "xof") {
    earnRows.push({ label: "Basic Salary", value: fmt(data.basic_salary, "XOF") });
    if (data.housing_allowance)   earnRows.push({ label: "Housing Allowance (20%)", value: fmt(data.housing_allowance, "XOF") });
    if (data.function_allowance)  earnRows.push({ label: "Function Allowance (5%)", value: fmt(data.function_allowance, "XOF") });
    if (data.transport_allowance) earnRows.push({ label: "Transport Allowance (5%)", value: fmt(data.transport_allowance, "XOF") });
    earnRows.push({ label: "Gross / Net Salary", value: fmt(data.gross_salary, "XOF"), bold: true });
  } else if (type === "rwf_wop") {
  ...
  if (type === "xof") {
    ...
    cell(doc, "Earnings", L + 6, startY + 6, fullW - 12, { bold: true, color: "#FFFFFF", size: 10 });
    ...
    for (let i = 0; i < tableRows; i++) {
      ...
      cell(doc, earnRows[i].value, L + fullW / 2, rowY + 4, fullW / 2 - 6, { bold: earnRows[i].bold, size: 9, align: "right" });
      if (i > 0) {
        doc.moveTo(L, rowY).lineTo(R, rowY).strokeColor("#EEEEEE").lineWidth(0.3).stroke();
      }
    }
    doc.moveTo(L + fullW / 2, bodyY).lineTo(L + fullW / 2, startY + tableH).strokeColor("#EEEEEE").lineWidth(0.5).stroke();
  ...
  if (type === "rwf" || type === "rwf_usd") {
    const hasEmployer = n(data.csr_employer) || n(data.occupational_hazards) || n(data.maternity_employer);
    if (hasEmployer) {
    ...
    if (hasEmployee) {
      ...
      if (n(data.csr_employee)) {
        dedRows.push({ label: "CSR 6% (RSSB)", value: fmt(data.csr_employee, "RWF"), indent: indentPx });
        totalDeductions += n(data.csr_employee);
      }
      if (n(data.maternity_employee)) {
        dedRows.push({ label: "Maternity 0.3%", value: fmt(data.maternity_employee, "RWF"), indent: indentPx });
    ...
    dedRows.push({ label: "WOP (RWF equivalent)", value: fmt(data.wop_rwf, "RWF"), indent: indentPx });
  ...
  cell(doc, "Deductions", rightX + 6, startY + 6, rightW - 6, { bold: true, color: "#FFFFFF", size: 10 });
  ...
  earnRows.forEach((row, i) => {
    ...
    cell(doc, row.value, L + leftW / 2, rowY, leftW / 2 - 4, { bold: row.bold, size: 9, align: "right" });
  });
  ...
  dedRows.forEach((row, i) => {
    ...
    if (row.isGroupTitle) {
      ...
      cell(doc, row.label, rightX + 4, rowY, rightW / 2 - 8, { size: 9, color: "#444444", indent: row.indent ?? 0 });
  ...
  doc.moveTo(L + leftW / 2, bodyY).lineTo(L + leftW / 2, startY + tableH).strokeColor("#EEEEEE").lineWidth(0.5).stroke();
  doc.moveTo(rightX + rightW / 2, bodyY).lineTo(rightX + rightW / 2, startY + tableH).strokeColor("#EEEEEE").lineWidth(0.5).stroke();
  for (let i = 1; i < tableRows; i++) {
    const lineY = bodyY + i * rh;
    doc.moveTo(L, lineY).lineTo(L + leftW, lineY).strokeColor("#EEEEEE").lineWidth(0.3).stroke();
    doc.moveTo(rightX, lineY).lineTo(rightX + rightW, lineY).strokeColor("#EEEEEE").lineWidth(0.3).stroke();
  }
...
function drawNetBox(doc: PDFKit.PDFDocument, label: string, value: string, y: number): number {
  ...
  doc.font("Helvetica-Bold").fontSize(13).fillColor(YELLOW)
    .text(value, L + 160, y + 9, { width: R - L - 170, align: "right", lineBreak: false });
  doc.font("Helvetica");
  return y + 40;
}

function drawDualNetBox(doc: PDFKit.PDFDocument, label: string, usdVal: string, rwfVal: string, y: number): number {
  ...
  doc.font("Helvetica").fontSize(9).fillColor("#CCFFCC")
...
function drawFooter(doc: PDFKit.PDFDocument, isRemittance: boolean) {
  ...
  doc.font("Helvetica").fontSize(8).fillColor("#888888")
    .text(
      ...
      L, footerY + 8, { align: "center", width: W, lineBreak: false },
    );
  doc.fontSize(8)
    .text("For inquiries, contact: info@ganzafrica.org", L, footerY + 20, { align: "center", width: W, lineBreak: false });
  doc.fontSize(7).fillColor("#BBBBBB")
    .text(
      `Generated on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}`,
      L, footerY + 32, { align: "center", width: W, lineBreak: false },
    );
}
...
export async function generatePayslipPDF(data: PayslipData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (type === "rwf_usd") {
        y = drawDualNetBox(doc, "Net Salary:", fmt(data.net_salary_usd, "USD"), fmt(data.net_salary, "RWF"), y + 6);
