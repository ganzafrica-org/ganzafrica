  ...
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      if (opportunityData.type === "fellowship" && opportunityData.fellowship_details) {
        const [fellowshipDetail] = await tx
          .insert(fellowship_details)
          .values({
        ...
        const [employmentDetail] = await tx
          .insert(employment_details)
          .values({
            opportunity_id: createdOpportunity.id,
      ...
      return {
        ...createdOpportunity,
        [opportunityData.type === "fellowship" ? "fellowship_details" : "employment_details"]:
          typeDetails,
      };
    });
  } catch (error) {
...
export async function getOpportunityById(id: number): Promise<any> {
  try {
    ...
    if (opportunity.type === "fellowship") {
      const fellowshipResult = await db
        .select()
    ...
    return {
      ...opportunity,
      [opportunity.type === "fellowship" ? "fellowship_details" : "employment_details"]:
        typeDetails,
    };
  } catch (error) {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      try {
        ...
        await tx.delete(applications).where(eq(applications.opportunity_id, id));

        // Delete fellowship details if they exist
        logger.info(`Deleting fellowship details for opportunity ${id}...`);
        await tx.delete(fellowship_details).where(eq(fellowship_details.opportunity_id, id));

        // Delete employment details if they exist
        logger.info(`Deleting employment details for opportunity ${id}...`);
        await tx.delete(employment_details).where(eq(employment_details.opportunity_id, id));
        ...
        throw new AppError(
          `Failed to delete opportunity and related records: ${errorMessage}`,
          500,
        );
      }
    });
    ...
    if (error && typeof error === "object" && "code" in error) {
      if (error.code === "23503") {
        ...
        throw new AppError(
          "Cannot delete opportunity: It has related records that cannot be removed",
          400,
        );
      }
    ...
    logger.error(`Full error details for opportunity ${id}:`, {
      ...
      detail:
        error && typeof error === "object" && "detail" in error ? (error as any).detail : "Unknown",
...
export async function listOpportunities(filters: OpportunityFilters): Promise<any[]> {
  try {
    ...
    const fellowshipDetailsResult = await db
      .select()
      .from(fellowship_details)
      .where(inArray(fellowship_details.opportunity_id, opportunityIds));

    const fellowshipDetailsMap = fellowshipDetailsResult.reduce(
      (map, detail) => {
    ...
    const employmentDetailsResult = await db
      .select()
      .from(employment_details)
      .where(inArray(employment_details.opportunity_id, opportunityIds));

    const employmentDetailsMap = employmentDetailsResult.reduce(
      (map, detail) => {
        map[detail.opportunity_id] = detail;
        return map;
      },
    ...
    if (applicationData.opportunity_id) {
      ...
      if (opportunity.custom_questions && opportunity.custom_questions.length > 0) {
        ...
        if (requiredQuestions.length > 0) {
          ...
          const missingRequiredQuestions: string[] = requiredQuestions.filter(
            (id: string) => !providedAnswers.includes(id),
          );
...
export async function listAllApplications(
  ...
  try {
    ...
    const opportunityIds = [
      ...new Set(
        applicationResults.filter((app) => app.opportunity_id).map((app) => app.opportunity_id),
      ),
    ];
      ...
      opportunityIds.length > 0
        ? await db
            ...
            .where(
              inArray(
                opportunities.id,
                opportunityIds.filter((id): id is number => id !== null),
              ),
            )
        : [];
    ...
    const enhancedApplications = applicationResults.map((app) => ({
      ...app,
      opportunity_title: app.opportunity_id
        ? opportunityMap[app.opportunity_id]?.title || "Unknown Opportunity"
        : "General Application",
    }));
