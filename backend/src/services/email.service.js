...
const logger = new config_1.Logger("EmailService");
// Create a nodemailer transporter
const transporter = nodemailer_1.default.createTransport({
    host: config_1.env.SMTP_HOST,
    port: config_1.env.SMTP_PORT,
    secure: config_1.env.SMTP_PORT === 465, // true for 465, false for other ports
    auth: {
        user: config_1.env.EMAIL_FROM,
...
async function sendEmail(to, subject, html) {
    try {
        const info = await transporter.sendMail({
            from: `"Ganzafrica" <${config_1.env.EMAIL_FROM}>`,
            to,
            subject,
            html,
        });
        ...
        throw new middlewares_1.AppError("Failed to send email", 500);
    }
}
// Send email verification email
async function sendVerificationEmail(to, data) {
    const verificationUrl = `${config_1.env.WEBSITE_URL}/verify-email?token=${data.token}`;
    const html = `
...
async function sendPasswordResetEmail(to, data) {
    const resetUrl = `${config_1.env.WEBSITE_URL}/reset-password?token=${data.token}`;
    const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
async function sendWelcomeEmail(to, name) {
    const loginUrl = `${config_1.env.WEBSITE_URL}/login`;
    const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
