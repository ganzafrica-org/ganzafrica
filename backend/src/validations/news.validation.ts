...
export const listNewsSchema = z.object({
  query: z.object({
    ...
    limit: z.string().refine(val => !isNaN(Number(val)), { message: 'Limit must be a number' }).optional(),
    offset: z.string().refine(val => !isNaN(Number(val)), { message: 'Offset must be a number' }).optional(),
