...
import { hr_assets, hr_asset_categories, hr_asset_specs, hr_asset_images, hr_users, hr_asset_maintenance } from "../../hr";
...
export async function seedAssets() {
  ...
  try {
    ...
    if (!phoneCatId || !laptopCatId || !chairCatId) {
      logger.warn("Required categories not found, skipping asset seed. Ensure categories are seeded first.");
      return;
    }
    ...
    for (const assetData of assetsToInsert) {
      ...
      const existing = await db.select().from(hr_assets).where(eq(hr_assets.serial_number, assetData.serial_number)).limit(1);
      ...
      if (assetData.serial_number === "SN-MACBOOK-001") {
        ...
        if (users.length > 0) {
          await db.insert(hr_asset_maintenance).values({
            asset_id: asset.id,
