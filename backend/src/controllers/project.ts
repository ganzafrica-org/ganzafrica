...
export const createProject = async (req: Request, res: Response) => {
    try {
        ...
        // Add this check
        const categoryExists = await db.select({ id: project_categories.id })
        ...
        const projectData = {
            ...
            members: req.body.members ? req.body.members.map((member: any) => ({
                ...member,
...
export const removeProjectMember = async (req: Request, res: Response) => {
    try {
        ...
        logger.error(`Remove project member error: ${req.params.id}/${req.params.userId}`, error);
        if (error instanceof AppError) {
            return res.status(error.statusCode).json({
                error: 'Project Member Error',
                message: error.message
